...
ggplot(Infer.diff) +
  ...
  labs(x = '', y = 'Increase in Probability of Acceptance', subtitle = expression('Accept if '*delta*' < 1'), color = 'Variable') +
  scale_y_continuous(expand = c(0, 0.05)) + scale_x_continuous(expand = c(0, 0)) +
  theme_bw()
...
ggplot(Infer.plt) +
  geom_path(mapping = aes(x = x, y = prob, color = cond)) +
  facet_wrap(var~., nrow = 2) + 
  labs(x = '', y = 'Probability of Acceptance', subtitle = expression('Accept if r < 1, f'[1]*' priority < 80%'), color = '') +
  scale_y_continuous(expand = c(0, 0.05)) + scale_x_continuous(expand = c(0, 0)) +
  theme_bw()
...
ggplot() +
  geom_contour(data = fine.grid, mapping = aes(x = x1, y = x2, z = dist), breaks = c(0.5, 1, 2, 4)) +
  theme_classic() +
  scale_x_continuous(expand = c(0, 0), limits = c(0, 5)) + scale_y_continuous(expand = c(0, 0), limits = c(0, 5))
ggplot() +
  geom_point(data = fine.grid, mapping = aes(x = x1, y = x2, color = (f1.norm < 1 & f2.norm < 1)))
...
names(GPar.all)
# Only want the input-output data: x1, x2, f1, f2
# GPar.all = GPar.all[,c('x1', 'x2', 'f1', 'f2')]
# names(GPar.all)
cluster = Mclust(data = GPar.all[,c('x1', 'x2', 'f1', 'f2')], G = 2:5)

# GPar.all = GPar.all[,c('x1', 'x2', 'f1', 'f2')]
# names(GPar.all)
cluster2d = Mclust(data = GPar.all[,c('x1', 'x2')], G = 2:5)
...
GPar.all$typ = cluster$classification
GPar.all$typ2 = cluster2d$classification
...
ggplot() +
  ...
  # Clustering
  geom_point(data = GPar.all, mapping = aes(x = x1, y = x2, shape = as.factor(typ)), size = 2.5) +

  labs(x = expression('x'[1]), y = expression('x'[2]), color = 'Acceptance Criteria', shape = 'Cluster') +
  scale_color_manual(values = c('delta' = 'red', 'cutof' = 'blue', 'radan' = 'green', 'Pareto' = 'black'),
                     labels = c('delta' = expression(delta*' < 1'), 'cutof' = expression('F'[1]^'*'*'< 1, F'[2]^'*'*'< 1'), 
                                'radan' = expression('r < 1, '*theta*' > 18'^'o'),
                                'Pareto' = expression('Pareto Front')),
...
ggplot() +
  ...
  geom_smooth(data = GPar.front, mapping = aes(x = x1, y = x2, color = 'Pareto'), 
              level = 0.95, formula = (y~x), method = 'loess') + 
  # Clustering
  geom_point(data = GPar.all, mapping = aes(x = x1, y = x2, shape = as.factor(typ2)), size = 2.5) +
...
```{r}
# Multivariate pdf of the 2nd cluster - the accepted cluster
mu = cluster$parameters$mean[,1]
Sigma = cluster$parameters$variance$sigma[,,1]
...
mu
Sigma
mvn.pdf.i(xi = mu, mu = mu, Sigma = Sigma)

```

Finegrid?
```{r}
# rng = seq(from = 0, to = 5, length.out = 50)
# new.pt = expand.grid(rng, rng)
# new.pt = data.frame(x1 = new.pt[,1],
#                     x2 = new.pt[,2])
# new.pt$f1 = f1(x1 = new.pt$x1, x2 = new.pt$x2)
# new.pt$f2 = f2(x1 = new.pt$x1, x2 = new.pt$x2)
# new.pt = rbind(new.pt, GPar.all[,c('x1', 'x2', 'f1', 'f2')])
# cluster.fine = Mclust(data = new.pt, G = 1:15)
# new.pt$typ = cluster.fine$classification
# cluster.fine = Mclust(data = GPar.all[,c('x1', 'x2', 'f1', 'f2')], G = 1:15)
# GPar.all$typ = cluster.fine$classification
ggplot() +
  geom_contour(data = fine.grid, mapping = aes(x = x1, y = x2, z = prob.delta), color = 'black', 
               breaks = c(0.5)) +
  geom_point(data = GPar.all, mapping = aes(x = x1, y = x2, color = as.factor(typ))) #+

ggplot() +
  geom_contour(data = fine.grid, mapping = aes(x = x1, y = x2, z = prob.delta), color = 'black', 
               breaks = c(0.5)) +
  geom_point(data = new.pt, mapping = aes(x = x1, y = x2, color = as.factor(typ))) #+

```

## Just categorize by output, then characterize the inputs?

```{r}
cluster.out = Mclust(data = GPar.all[,c('f1', 'f2')], G = 2:5)
...
ggplot() +
  ...
  geom_smooth(data = GPar.front, mapping = aes(x = x1, y = x2, color = 'Pareto'), 
              level = 0.95, formula = (y~x), method = 'loess') + 
  # Clustering
  geom_point(data = GPar.all, mapping = aes(x = x1, y = x2, shape = as.factor(typ.out)), size = 2.5) +
...
```{r}
# Predict fine grid split
res = predict(object = cluster.out, newdata = fine.grid[,c('f1', 'f2')])
fine.grid$class = res$classification
fine.grid$class[fine.grid$class != 2] = 0

ggplot(fine.grid) +
  geom_contour(data = fine.grid, mapping = aes(x = x1, y = x2, z = dist, color = 'tru'),
               breaks = c(1), size = 1.5) +
  geom_contour(mapping = aes(x = x1, y = x2, z = class, color = 'GM')) +
  geom_contour(data = fine.grid, mapping = aes(x = x1, y = x2, z = prob.delta, color = 'GP'),
               breaks = c(0.5), size = 1.5) +

  # Clustering
  geom_point(data = GPar.all, mapping = aes(x = x1, y = x2, shape = as.factor(typ.out)), size = 2.5) +
  labs(x = 'x1', y = 'x2', shape = 'GM Class') +
  scale_color_manual(name = '', values = c('tru' = 'black', 'GP' = 'red', 'GM' = 'maroon'), 
                     labels = c('tru' = 'True boundary', 'GP' = 'Gaussian Process', 'GM' = 'Gaussiam Mixture'), 
                     breaks = c('tru', 'GP', 'GM')) +
  theme_classic()

ggplot(fine.grid) +
  geom_contour(data = fine.grid, mapping = aes(x = f1, y = f2, z = dist, color = 'tru'),
               breaks = c(1), size = 1.5) +
  geom_contour(mapping = aes(x = f1, y = f2, z = class, color = 'GM')) +
  geom_contour(data = fine.grid, mapping = aes(x = f1, y = f2, z = prob.delta, color = 'GP'),
               breaks = c(0.5), size = 1.5) +

  # Clustering
  geom_point(data = GPar.all, mapping = aes(x = f1, y = f2, shape = as.factor(typ.out)), size = 2.5) +
  labs(x = 'x1', y = 'x2', shape = 'GM Class') +
  scale_color_manual(name = '', values = c('tru' = 'black', 'GP' = 'red', 'GM' = 'maroon'), 
                     labels = c('tru' = 'True boundary', 'GP' = 'Gaussian Process', 'GM' = 'Gaussiam Mixture'), 
                     breaks = c('tru', 'GP', 'GM')) +
  theme_classic() + ylim(c(0, 50))

```

Probability mass function marginalization?

The GM uses the f1 and f2 as inputs rather than x1 and x2. Descriptive statistics on the x1 and x2?

```{r}
c(mean(fine.grid$x1[fine.grid$class == 2]), mean(fine.grid$x1[fine.grid$class != 2]), 
  mean(fine.grid$x2[fine.grid$class == 2]), mean(fine.grid$x2[fine.grid$class != 2]))
c(sd(fine.grid$x1[fine.grid$class == 2]), sd(fine.grid$x1[fine.grid$class != 2]),
  sd(fine.grid$x2[fine.grid$class == 2]), sd(fine.grid$x2[fine.grid$class != 2]))

ggplot(fine.grid) +
  geom_density(mapping = aes(x = x1, color = as.factor(class)), linetype = 1) +
  geom_density(mapping = aes(x = x2, color = as.factor(class)), linetype = 2)

# Since uniform sampling is possible, Bayes rule is not useful. 
# As the number of dimensions increases, this becomes less useful. 
# eg. Estimates from the sampled points must be corrected:

c(mean(GPar.all$x1[GPar.all$typ.out == 2]), mean(GPar.all$x1[GPar.all$typ.out != 2]), 
  mean(GPar.all$x2[GPar.all$typ.out == 2]), mean(GPar.all$x2[GPar.all$typ.out != 2]))
c(sd(GPar.all$x1[GPar.all$typ.out == 2]), sd(GPar.all$x1[GPar.all$typ.out != 2]),
  sd(GPar.all$x2[GPar.all$typ.out == 2]), sd(GPar.all$x2[GPar.all$typ.out != 2]))

ggplot(GPar.all) +
  geom_density(mapping = aes(x = x1, color = as.factor(typ.out == 2)), linetype = 1) +
  geom_density(mapping = aes(x = x2, color = as.factor(typ.out == 2)), linetype = 2)

```

```{r}
# Applying Bayes rule to find the cdf
# P[A|B] = P[B|A] P[A]/P[B]
x.rng = seq(from = 0, to = 5, length.out = 100)
cdf.x1 = c(); cdf.x2 = c()
for(i in 1:length(x.rng)){
  cdf.x1[i] = nrow(filter(GPar.all, x1 <= x.rng[i], typ.out == 2))/nrow(filter(GPar.all, typ.out == 2)) * 
    nrow(filter(GPar.all, typ.out == 2))/nrow(GPar.all) *
    nrow(GPar.all)/nrow(filter(GPar.all, x1 <= x.rng[i]))
  cdf.x2[i] = nrow(filter(GPar.all, x2 <= x.rng[i], typ.out == 2))/nrow(filter(GPar.all, typ.out == 2)) * 
    nrow(filter(GPar.all, typ.out == 2))/nrow(GPar.all) *
    nrow(GPar.all)/nrow(filter(GPar.all, x2 <= x.rng[i]))
}

ggplot(data.frame(x.rng, cdf.x1, cdf.x2)) +
  geom_path(mapping = aes(x = x.rng, y = cdf.x1)) +
  geom_path(mapping = aes(x = x.rng, y = cdf.x2), color = 'red')
# Estimate the derivative to get pdf
ggplot(data.frame(x.rng = x.rng[2:100], cdf.x1 = cdf.x1[2:100] - cdf.x1[1:99], cdf.x2 = cdf.x2[2:100] - cdf.x2[1:99])) +
  geom_smooth(mapping = aes(x = x.rng, y = cdf.x1), color = 'blue',
              level = 0.1, formula = y~x, method = 'loess') +
  geom_smooth(mapping = aes(x = x.rng, y = cdf.x2), color = 'red',
              level = 0.1, formula = y~x, method = 'loess') +
  labs(x = 'x', y = 'density')
  # geom_point(mapping = aes(x = x.rng, y = cdf.x1), color = 'blue') +
  # geom_point(mapping = aes(x = x.rng, y = cdf.x2), color = 'red')
```
Reapply Bayes rule with additional sampling? would require
```{r}
x.rng = seq(from = 0, to = 5, length.out = 100)
cdf.x1 = c(); cdf.x2 = c()
for(i in 1:length(x.rng)){
...
# Estimate the derivative to get pdf
ggplot(data.frame(x.rng = x.rng[2:100], cdf.x1 = cdf.x1[2:100] - cdf.x1[1:99], cdf.x2 = cdf.x2[2:100] - cdf.x2[1:99])) +
  geom_smooth(mapping = aes(x = x.rng, y = cdf.x1), level = 0.1, formula = y~x, method = 'loess') +
  geom_smooth(mapping = aes(x = x.rng, y = cdf.x2), color = 'red',
              level = 0.1, formula = y~x, method = 'loess')
