...
The regression shows a strong relationship to $x_1$ and weak relationship to $x_2$, although part of this is due to the skew from a few extreme values from the samples.
It is not unreasonable to expect the results here to change if certain regions were sampled more or less.
Therefore, a better method of importance ranking reflects the model, not the training data.
...
ggplot() +
  ...
  scale_fill_manual(name = 'Variable', values = c('x1' = 'red', 'x2' = 'blue'), 
                    labels = c('x1' = expression('x'[1]), 'x2' = expression('x'[2]))) +
    theme_classic() + theme(legend.position = c(0.85, 0.9), legend.background = element_rect(fill = 'white'), 
                          legend.title = element_blank(),
                          title = element_text(size = 18),
                          legend.text = element_text(size = 18),
                          axis.title = element_text(size = 18),
                          axis.text = element_text(size = 18)) + guides(fill = FALSE)
ggsave('/home/jkboualavong/Desktop/PSU/Data202102_Paper2_BayesOpt/Figures/Quartic_1MarginSD.jpg', width = 8, height = 6)
...
However, this highlights how much closer these two variables are in their relative importance, despite $x_1$ have a 
...
In this case, due to symmetry, there is similarity between x1 and x2's behavior as single variables and when conditioned on the other. 
There is a slightly greater improvement to $x_2$ when knowing $x_1$ than the inverse, matching the slightly greater importance of $x_1$.
...
When comparing this regression and slope to that of the previous input, despite being a reflection of the exact same variables, these values are dramatically different due to differences in what regions were sampled. 
While it reaches the same conclusion, it highlights how the method of using regressions and slopes can be misleading and unstable.
