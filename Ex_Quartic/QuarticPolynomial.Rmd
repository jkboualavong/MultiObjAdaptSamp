...
While adaptive sampling improves the results compared to without any additional samples, random sampling sometimes appears to improve the result more for $x_1$.
...
As expected from the importance ranking, conditioning $x_2$ on $x_1$ leads to a larger improvement in the probability of acceptance than the other way around.
...
As indicated by this acceptance criteria, the boundary of the set is well defined with just the Pareto frontier, so additional iterations are not as necessary. Continuing with the procedure to show the feature importance and acceptance probability procedures.
...
In general, it appears the local maximum that happens when $x_2 = 3$ causes issues for estimating their marginals, with a consistent underestimate.
...
The marginalized probability here is generally an underestimate of the expected probability, particularly for $x_2$. Adaptive sampling appears to qualitatively improve the results, but the difference is small.
The relative importance of $x_1$ and $x_2$ flip in this criteria, showing how the relative importance is dependent on the question being asked.
...
```
Oddly, while the importance ranking for $x_2$ was greater than that of $x_1$, conditioning $x_2$ on $x_1$ still led to the greatest improvement. However, the fact that the importance rankigns were very similar to each other may play a part in it; propagating the uncertainty in the uncertainty estimates would be helpful in discerning if the difference is actually substantial enough to claim that $x_2$ was in fact more important.
