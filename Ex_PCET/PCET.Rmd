...
The hyperparameters L, k, and x0 of the weighting function were tuned by fitting to a variety of minimum lean gas pressures on the range of 0.016 to 2.5 atm and then selected as the modal value.
...
```{r PCET Derived Functions: Energy Deman}
...
# # Demo of the weight effects: insufficient capture
# pCO2.test = 10^seq(from = -4, to = log(0.15, base = 10), by = 0.1)
# plot(pCO2.test, weight.fun(pCO2.lean = pCO2.test))
# # When the minimum lean gas pressure exceeds the inlet pressure, then a negative energy is obtained, so the weight should be negative
# pCO2.test = 10^seq(from = log(0.16, base = 10), to = 1, by = 0.1)
# plot(pCO2.test, weight.fun(pCO2.lean = pCO2.test))
...
```{r}
# design.start$x = 1:nrow(design.start)
# result.start$x = 1:nrow(result.start)
# des = merge(design.start, result.start, by = 'x')
des = design.start
des$DIC.cyc = NaN; des$pCO2.lean = NaN
for(i in 1:nrow(des)){
  des$DIC.cyc[i] = DIC.diff(Na = des$Na.A[i]*10^des$logA[i], 
                                 A = 10^des$logA[i], 
                                 beta1 = 0, beta2 = 0, 
                                 k1 = 10^-des$pka1[i], 
                                 k2 = 10^-(des$pka2[i] + des$pka1[i]), 
                                 pCO2.in = 0.15, pCO2.out = 1)
  # if(des$DIC.cyc[i] > 0){
    des$pCO2.lean[i] = pCO2.lean(Na = des$Na.A[i]*10^des$logA[i], 
                                      A = 10^des$logA[i], 
                                      beta1 = 0, beta2 = 0, 
                                      k1 = 10^-des$pka1[i], 
                                      k2 = 10^-(des$pka2[i] + des$pka1[i]), 
...
ggplot(filter(des, Energy.kJ.mol < 100)) +
  geom_point(mapping = aes(x = Flux.mol.m2s, y = Energy.kJ.mol))

des
ggplot(filter(des)) +
  geom_point(mapping = aes(x = pCO2.lean, y = Flux.mol.m2s)) +
  facet_grid((pCO2.lean >= 0.015)~(Flux.mol.m2s > 0), scales = 'free') +
  scale_x_log10()
...
```{r}
# Calculate total CO2 per cycle; minimum lean gas
GPar.all$pCO2.lean = NaN; GPar.all$DIC.cyc = NaN;
for(i in 1:nrow(GPar.all)){
  GPar.all$DIC.cyc[i] = DIC.diff(Na = GPar.all$Na.A[i]*10^GPar.all$logA[i], 
                                 A = 10^GPar.all$logA[i], 
                                 beta1 = 0, beta2 = 0, 
                                 k1 = 10^-GPar.all$pka1[i], 
                                 k2 = 10^-(GPar.all$pka2[i] + GPar.all$pka1[i]), 
                                 pCO2.in = 0.15, pCO2.out = 1)
  # if(GPar.all$DIC.cyc[i] > 0){
    GPar.all$pCO2.lean[i] = pCO2.lean(Na = GPar.all$Na.A[i]*10^GPar.all$logA[i], 
                                      A = 10^GPar.all$logA[i], 
                                      beta1 = 0, beta2 = 0, 
                                      k1 = 10^-GPar.all$pka1[i], 
                                      k2 = 10^-(GPar.all$pka2[i] + GPar.all$pka1[i]), 
...
```{r}
ggplot() +
  geom_point(data = filter(GPar.all, Flux.mol.m2s > 0, Energy.kJ.mol < 100), mapping = aes(x = Flux.mol.m2s, y = Energy.kJ.mol)) +
  geom_point(data = filter(GPar.front, Flux.mol.m2s > 0, Energy.kJ.mol < 100), mapping = aes(x = Flux.mol.m2s, y = Energy.kJ.mol), color = 'red') +
  geom_line(data = filter(GPar.front, Flux.mol.m2s > 0, Energy.kJ.mol < 100), mapping = aes(x = Flux.mol.m2s, y = Energy.kJ.mol), color = 'red')
...
ggplot() +
  geom_point(data = filter(GPar.all, Energy.kJ.mol < 100), mapping = aes(x = Flux.mol.m2s, y = Energy.kJ.mol)) +
  geom_point(data = filter(GPar.front, Energy.kJ.mol < 100), mapping = aes(x = Flux.mol.m2s, y = Energy.kJ.mol), color = 'red') +
  geom_line(data = filter(GPar.front, Energy.kJ.mol < 100), mapping = aes(x = Flux.mol.m2s, y = Energy.kJ.mol), color = 'red')

```

```{r}
filter(GPar.front, Flux.mol.m2s >= 0)
```


Lean pressures above 0.015 should be showing the flux going the wrong direction, but they are not exclusively doing so. Why? Is the enhancement factor calculated as negative?

```{r}
kinetic.force2 = function(k1, k2, beta1, beta2, A.tot, Na, pCO2.in, pCO2.out){
  # Constants
  xA.lim = c(0.025, 0.975)
  kH = 3.4e-2; # M/atm
  # Calculate the pCO2 of the fully reduced species prior to equilibration with the gas
...
```{r}
test = filter(des, (Flux.mol.m2s > 0), (pCO2.lean >= 0.015))[1:2,]
test
kinetic.force(k1 = 10^-test$pka1[1], k2 = 10^-(test$pka1[1] + test$pka2[1]),
               beta1 = 0, beta2 = 0, 
               A.tot = 10^test$logA[1], Na = 10^test$logA[1]*test$Na.A[1], 
               pCO2.in = 0.15, pCO2.out = 1)
kinetic.force(k1 = 10^-test$pka1[2], k2 = 10^-(test$pka1[2] + test$pka2[2]),
               beta1 = 0, beta2 = 0, 
               A.tot = 10^test$logA[2], Na = 10^test$logA[2]*test$Na.A[2], 
               pCO2.in = 0.15, pCO2.out = 1)

as.matrix(test)[,2:5]
PCET.obj.flu(inputs = as.matrix(test)[,2:5])
...
```{r}
ggplot(GPar.all) +
  geom_density(mapping = aes(x = logA))
ggplot(GPar.all) +
  geom_density(mapping = aes(x = pka1)) +
  geom_density(mapping = aes(x = pka1+pka2), color = 'red')
ggplot(GPar.all) +
  geom_density(mapping = aes(x = pka2))
ggplot(GPar.all) +
  geom_density(mapping = aes(x = Na.A))
