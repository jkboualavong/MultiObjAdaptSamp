...
fill.sample.delta = function(x, model){
  ...
  prob = pnorm(q = 0, mean = res$mean - 3, sd = res$sd)
...
fill.sample.cutof = function(x, model.f1, model.f2){
  ...
  # Probability distribution fits a Gaussian distribution
  prob = pnorm(q = 0, mean = res.f1$mean - 3, sd = res.f1$sd) * 
    pnorm(q = 0, mean = res.f2$mean - 3, sd = res.f2$sd)
  ...
  for(j in 1:2){
    for(k in 1:2){
      des.start = rbind(des.start,
        data.frame(x1 = runif(n = npt, min = bounds[i], max = bounds[i+1]),
                   x2 = runif(n = npt, min = bounds[j], max = bounds[j+1]),
                   x3 = runif(n = npt, min = bounds[k], max = bounds[k+1])))
...
Based on this Monte Carlo estimate, the cutoff value should be approximately 3 for all 3 criteria to guarantee 7~10% of the total hypervolume space is accepted.
...
For this example, defines $\delta$ as the distance between the point and the point on the Pareto frontier with the same value of theta. Accept only points with $\delta < 3$.

```{r Load data: Accept Distance less than 3}
# Load data
...
```{r Iteration 1: Distance less than 3, warning=FALSE, message=FALSE}
