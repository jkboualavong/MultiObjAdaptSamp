...
8. Calculate the variance of the 1-variable conditional probability expectation value at each input value. The area under the variance curve indicates the relative importance of the other variables.
...
The domain of both $x_1$ and $x_2$ is [0,1]. 
...
3. Calculate the normalized distance in the objective space and f1/f2 prioritization of each point to the Pareto front. These two metrics will be used to establish the acceptable suboptimal performance and the relative importance of each input variable.

Normalization is defined such that (0,0) is the utopia point and (1,0) and (0,1) are the single-objective optimizations. The prioritization is the angle $\theta$ made between the point in the normalized objective space and the x-axis, i.e. the axis of purely prioritizing objective 2. This angle is re-mapped to a percentage from 0 to 100% prioritization of objective 2. The distance between each point and the Pareto front is the distance in the normalized space
...
Determine the expected 1-variable marginals with the above criteria for comparison.
...
For the purposes of illustration, P[accept | $x_1$], P[accept | $x_2$], and P[accept | $x_3$] will be shown to show how the regression is consistent with the probabilities.
...
This indicates that the sample biasing that caused the correlation coefficients and slopes of $x_2$ and $x_3$ to differ propagated to the GP model.
This is likely because $x_3$ only shows 1 peak in the marginal distribution as compared to $x_2$, which was able to capture the local minima.
As a result, the area of $x_3$ is much smaller since its optimal value narrows the range from its full capacity of [0, 1] to about [0.4, 0.6].

## IV: Suggested ranges

Due to the degeneracy of $x_2$ and $x_3$ and the discontinuous domains, this set of objective functions is not appropriate illustrations of the method for finding the suggested ranges of each input variable.
Therefore, while it is possible to find the values are would yield viable results, there is no clear benefit to performing the calculation.
