...
for(i in 1:length(mod.ener.a.rel)){
  ...
  ener[,i+length(mod.ener.a.rel)] = fill$mean
...
# Add a target point based on the existing data: higher than desired energy, lower flux, but still captures CO2
new = filter(GPar.nat, Energy.kJ.mol > 25, Energy.kJ.mol < 40, Flux.mol.m2s > 0, Flux.mol.m2s < 0.1*max(GPar.front$Flux.mol.m2s))
new$label = 'Bad Result'
new$acid = -1
if(new$typ == '+Base'){new$acid = 1}
test.sys = rbind(test.sys, new[, names(new) %in% names(test.sys)])
...
write.csv(test.sys[c(1, 8, 9), !names(test.sys) %in% 'dist'], 'Cycle_PointsOfInterest.csv')
