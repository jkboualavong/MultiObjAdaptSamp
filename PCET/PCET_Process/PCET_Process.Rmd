...
# Description
This notebook takes the results from PCET optimization and sampling and interrogates specific points to illustrate 
...
The training data uses the datasets calculated from the scripts in the Ex_PCET_Acid and Ex_PCET_Base folders.
...
## Parameter space
Re-plotting the dataset from Huynh et al. J Am Chem Soc. 2016 December 14; 138(49): 15903–15910. doi:10.1021/jacs.6b05797.

The data will be analyzed again later in the context of CO2 capture performance; these plots simply re-iterate the motivation for the constraints on the pKas.
...
## CO2 Capture Model functions

The functions are written assuming the pH correction is a base, but simply changing the sign to negative will treat it as if an acid were added.
...
To obtain a complete description of the system, results of models trained on each dataset (+acid or +base) will be interpreted together.
...
# Load basic data
GPar.all.base = read.csv(file = '../Ex_PCET_Basic/GPar_SubRef_data.csv')
...
The selection criteria for a competitive result:
* energy demand < ~35 kJ/mol C to be competitive with temperature-swing processes
* flux > 1/10th of the flux of MEA capture processes (calculated to be ~22 mmol/m2*s)
...
reg.full = data.frame(min.flux = 2.2e-3,
                     max.flux = max(GPar.front$Flux.mol.m2s),
                     min.ener = min(GPar.front$Energy.kJ.mol), max.ener = 33)
# Region 2: the right-hand side of high flux but moderate energy, i.e. flux > 22 mmol/m2s. Increase the energy above 45 to capture the Pareto front there
reg.flux = data.frame(min.flux = 22e-3, max.flux = max(GPar.front$Flux.mol.m2s),
                     min.ener = min(GPar.front$Energy.kJ.mol),
                     max.ener = 45)
# Region 3: the bottom section of low energy but moderate flux
reg.ener = data.frame(min.flux = 2.2e-3,
                     max.flux = max(GPar.front$Flux.mol.m2s), 
                     min.ener = min(GPar.front$Energy.kJ.mol),
                     max.ener = mean(range(filter(GPar.front, Flux.mol.m2s < 24e-3)$Energy.kJ.mol)))
# Plotting
ggplot() +
  geom_rect(reg.full, 
...
The Pareto front has a slight concavity where the energy demand increases but the flux remains constant; coincidentally, this happens at around the flux of MEA capture.
...
Partial data cleaning may be needed: there are samples where the flux is very strongly negative (less than -10 mol/m2s), which sometimes skews the results.
However, the flux cannot be converted to a log scale as the energy was because it contains fluxes at or around 0.
Instead, a partial sample of points will be used to generate a collection of models, whose average performance will be used as the estimate.
This was also some inconsistency between runs due to slight differences in the random number seed, suggesting either poor convergence or degeneracy of the GP models.
Regardless, aggregation of multiple models trained on subsets of the data should help make results more consistent.

In this case, the mean probability and standard error of the mean will be used as the metrics for importance ranking, as well as for determining suggested ranges.
This repeated testing should minimize the run-to-run variance in the probability estimations.

Adequate sampling requires at least 60% of the dataset, as this guarantees that at least some of the points in the training set have positive fluxes and/or low energy demands.

Combining the acid and base datasets leads to convergence issues even if a fifth variable of whether acid or base is included in the model.
As a result, half of the models will be generated with acid conditions and half with basic conditions, and they will handled separated until aggregation of the probabilities at the end.

There is interest in obtaining not just the parameters for the natural variables, but also for the relative variables (pKa difference and acid or base to quinone ratio), as those may be more insightful.
...
mod.flux.a.rel = list(); mod.ener.a.rel = list()
mod.flux.b.rel = list(); mod.ener.b.rel = list()

row.frac = 0.6
# row.frac = 1
for(i in 1:nmod){
  # Base: natural variables
  GPar.samp = filter(GPar.nat, typ == 1)
  nsamp = ceiling(nrow(GPar.samp)*row.frac)
  GPar.samp = GPar.samp[sample(x = 1:nrow(GPar.samp), size = nsamp), ]
  ...
  GPar.samp = filter(GPar.all, typ == '+Base')
  nsamp = ceiling(nrow(GPar.samp)*row.frac)
  GPar.samp = GPar.samp[sample(x = 1:nrow(GPar.samp), size = nsamp), ]
  mod = fill.sample.flux(GPar.data = GPar.samp, input.name = c('pka1', 'pka2', 'logA', 'Na.A'),
                             output.name = 'Flux.mol.m2s')
  mod.flux.b.rel = append(mod.flux.b.rel, mod)
  mod = fill.sample.ener(GPar.data = GPar.samp,
                              input.name = c('pka1', 'pka2', 'logA', 'Na.A'),
                              output.name = 'Energy.kJ.mol')
  mod.ener.b.rel = append(mod.ener.b.rel, mod)
  # Acid: relative variables
  GPar.samp = filter(GPar.all, typ == '+Acid')
  nsamp = ceiling(nrow(GPar.samp)*row.frac)
  GPar.samp = GPar.samp[sample(x = 1:nrow(GPar.samp), size = nsamp), ]
  mod = fill.sample.flux(GPar.data = GPar.samp, input.name = c('pka1', 'pka2', 'logA', 'Na.A'),
                             output.name = 'Flux.mol.m2s')
  mod.flux.a.rel = append(mod.flux.a.rel, mod)
  mod = fill.sample.ener(GPar.data = GPar.samp,
                              input.name = c('pka1', 'pka2', 'logA', 'Na.A'),
                              output.name = 'Energy.kJ.mol')
  mod.ener.a.rel = append(mod.ener.a.rel, mod)
}
...
# Output space characterization

How likely a randomly selected set of inputs will generate good or bad results? What the average result would be? What is the range of possible results?
This will be assessed using the relative variable functions since they are easier to sample from (rectangular constraints), and the quality difference among the GP with initial testing did not appear to be substantially different.
These questions will be answered using the means among the different GP trained on different subsets, with the belief that the average among them will converge on the true expected mean.

That is, for a randomly sampled point, for each model:
* Determine its energy and flux
* Determine its probability of being competitive with MEA, high flux, or low energy
This will generate a mean and variance for each point, which, with repeated sampling, can give a sense of the entire space.

```{r}
# Create a set of random points based on the limits
MCsamp = 5e4
rand.samp = data.frame(pka1 = runif(n = MCsamp, min = min(GPar.all$pka1), max = max(GPar.all$pka1)),
                       pka2 = runif(n = MCsamp, min = min(GPar.all$pka2), max = max(GPar.all$pka2)),
                       logA = runif(n = MCsamp, min = min(GPar.all$logA), max = max(GPar.all$logA)),
                       Na.A = runif(n = MCsamp, min = min(GPar.all$Na.A), max = max(GPar.all$Na.A)))
# Mean and ranges
ener = matrix(rep(0, 2*length(mod.ener.a.rel)*nrow(rand.samp)), nrow = nrow(rand.samp))
flux = ener
# Probabilities: competitive with MEA, low energy, high flux, and at least 90% capture
p.acpt = ener; p.ener = ener; p.flux = ener; p.capt = ener;
# Probabilities

for(i in 1:length(mod.ener.a.rel)){
  # Energies
  fill.e = predict(mod.ener.a.rel[[i]], newdata = rand.samp, type = 'UK')
  ener[,i] = fill.e$mean
  # Fluxes
  fill.f = predict(mod.flux.a.rel[[i]], newdata = rand.samp, type = 'UK')
  flux[,i] = fill.f$mean
  # Probabilities
  p.acpt[,i] = (1 - pnorm(q = 0, mean = fill.f$mean - reg.full$min.flux, sd = fill.f$sd)) *
        pnorm(q = 0, mean = fill.e$mean - log10(reg.full$max.ener), sd = fill.e$sd)
  p.ener[,i] = (1 - pnorm(q = 0, mean = fill.f$mean - reg.ener$min.flux, sd = fill.f$sd)) *
        pnorm(q = 0, mean = fill.e$mean - log10(reg.ener$max.ener), sd = fill.e$sd)
  p.flux[,i] = (1 - pnorm(q = 0, mean = fill.f$mean - reg.flux$min.flux, sd = fill.f$sd)) *
        pnorm(q = 0, mean = fill.e$mean - log10(reg.flux$max.ener), sd = fill.e$sd)
  p.capt[,i] = (1 - pnorm(q = 0, mean = fill.f$mean, sd = fill.f$sd))
  # Repeat with base
  fill.e = predict(mod.ener.b.rel[[i]], newdata = rand.samp, type = 'UK')
  ener[,i+length(mod.ener.a.rel)] = fill.e$mean
  fill.f = predict(mod.flux.b.rel[[i]], newdata = rand.samp, type = 'UK')
  flux[,i+length(mod.ener.a.rel)] = fill.f$mean
  p.acpt[,i+length(mod.ener.a.rel)] = 
    (1 - pnorm(q = 0, mean = fill.f$mean - reg.full$min.flux, sd = fill.f$sd)) *
        pnorm(q = 0, mean = fill.e$mean - log10(reg.full$max.ener), sd = fill.e$sd)
  p.ener[,i+length(mod.ener.a.rel)] = 
    (1 - pnorm(q = 0, mean = fill.f$mean - reg.ener$min.flux, sd = fill.f$sd)) *
        pnorm(q = 0, mean = fill.e$mean - log10(reg.ener$max.ener), sd = fill.e$sd)
  p.flux[,i+length(mod.ener.a.rel)] = 
    (1 - pnorm(q = 0, mean = fill.f$mean - reg.flux$min.flux, sd = fill.f$sd)) *
        pnorm(q = 0, mean = fill.e$mean - log10(reg.flux$max.ener), sd = fill.e$sd)
  p.capt[,i+length(mod.ener.a.rel)] = 
    (1 - pnorm(q = 0, mean = fill.f$mean, sd = fill.f$sd))
}
rm(fill.e, fill.f)
# Set of possible energies and fluxes
rand.samp$ener = 10^apply(ener, 1, mean)
rand.samp$flux = apply(flux, 1, mean)
rand.samp$p.acpt = apply(p.acpt, 1, mean)
rand.samp$p.ener = apply(p.ener, 1, mean)
rand.samp$p.flux = apply(p.flux, 1, mean)
rand.samp$p.capt = apply(p.capt, 1, mean)
rm(ener, flux, p.acpt, p.ener, p.flux, p.capt)
# Mean energy
descr = rbind(apply(as.matrix(rand.samp), 2, range), 
      apply(as.matrix(rand.samp), 2, mean),
      apply(as.matrix(rand.samp), 2, sd),
      apply(as.matrix(rand.samp), 2, median))
descr = descr[, 5:ncol(descr)]
row.names(descr) = c('Min', 'Max', 'Mean', 'SD', 'Median')
descr

g1 = ggplot(rand.samp) +
  geom_density(mapping = aes(x = ener)) +
  labs(x = 'Energy Demand', y = 'Relative Frequency') +
  scale_y_continuous(labels = NULL) +
  scale_x_log10()
g2 = ggplot(rand.samp) +
  geom_density(mapping = aes(x = flux)) +
  labs(x = 'Flux', y = 'Relative Frequency') +
  scale_y_continuous(labels = NULL)
g1 / g2

g1 = ggplot(rand.samp) +
  geom_density(mapping = aes(x = p.capt)) +
  labs(x = 'Proc: Capture', y = 'Relative Frequency') +
  scale_y_continuous(labels = NULL)
g2 = ggplot(rand.samp) +
  geom_density(mapping = aes(x = p.acpt)) +
  labs(x = 'Prob: Competitive', y = 'Relative Frequency') +
  scale_y_continuous(labels = NULL)
g3 = ggplot(rand.samp) +
  geom_density(mapping = aes(x = p.ener)) +
  labs(x = 'Prob: Low Energy', y = 'Relative Frequency') +
  scale_y_continuous(labels = NULL)
g4 = ggplot(rand.samp) +
  geom_density(mapping = aes(x = p.flux)) +
  labs(x = 'Prob: High flux', y = 'Relative Frequency') +
  scale_y_continuous(labels = NULL)
(g1 + g2) / (g3 + g4)
rm(g1, g2, g3, g4)

write.csv(rand.samp[, names(rand.samp) %in% c('ener', 'flux')], 'OutcomeDistribution.csv')
write.csv(descr, 'OutcomeDescribe.csv')
rm(rand.samp, descr)
```

The distributions for the energy demand and flux appear don't capture the entire range of possibilities from the training dataset: the flux does not extend as low (approximately -10 on this scale), while the energy demand does not extend as high (10^7).
This suggests that the distributions are more centrally-focused, or that those extrema were rare events found by exploratory sampling.
This does, however, provide some indication regarding the relative distributions close to the modal value: the energy demand tends to be focused in the 30-100 kJ/mol range, while the flux tends to be slightly positive.

Looking at the relative frequencies of the sampled probabilities, there is some interesting multimodality in all distributions except the high flux condition.
This suggests that there are clusters of points that are likely to meet the specified criteria, and another cluster of points that are less likely.
...
We use the average variance to be able to compare continuous variables to categorical variables.
...
For ease of repeatability, writing a set of functions.
...
marginals1D = function(E.cutof, F.cutof, mod.flux.a, mod.ener.a, mod.flux.b, mod.ener.b){
  ...
  post.optim = data.frame(pka1 = seq(from = pka1.rng[1], to = pka1.rng[2], length.out = resolution),
                          ...
                          Na.A = seq(from = Na.A.rng[1] + logA.rng[1], to = Na.A.rng[2] + logA.rng[2], 
                                     length.out = resolution),
          # Combined acid and base for importance ranking
          ...
          # Acid and base separated for marginalization plotting
          pa.pka1 = NaN, pa.pka2 = NaN, pa.logA = NaN, pa.Na.A = NaN, # Probability acceptance median
          sa.pka1 = NaN, sa.pka2 = NaN, sa.logA = NaN, sa.Na.A = NaN, # variance for importance ranking
          pb.pka1 = NaN, pb.pka2 = NaN, pb.logA = NaN, pb.Na.A = NaN, # Probability acceptance median
          sb.pka1 = NaN, sb.pka2 = NaN, sb.logA = NaN, sb.Na.A = NaN, # variance for importance ranking
  ...
  for(i in 1:resolution){
    ...
    # Acid/base for marginal
    post.optim$pa.pka1[i] = mean(p.a);  post.optim$pb.pka1[i] = mean(p.b)  
    post.optim$sa.pka1[i] = sd(p.a);  post.optim$sb.pka1[i] = sd(p.b)
    ...
    # Acid/base for marginal
    post.optim$pa.pka2[i] = mean(p.a);  post.optim$pb.pka2[i] = mean(p.b)  
    post.optim$sa.pka2[i] = sd(p.a);  post.optim$sb.pka2[i] = sd(p.b)
    ...
    # Acid/base for marginal
    post.optim$pa.logA[i] = mean(p.a);  post.optim$pb.logA[i] = mean(p.b)  
    post.optim$sa.logA[i] = sd(p.a);  post.optim$sb.logA[i] = sd(p.b)
    ...
    # Acid/base for marginal
    post.optim$pa.Na.A[i] = mean(p.a);  post.optim$pb.Na.A[i] = mean(p.b)  
    post.optim$sa.Na.A[i] = sd(p.a);  post.optim$sb.Na.A[i] = sd(p.b)
...
```{r Reload natural variable data}
...
```{r 1D Marginals: natural variables}
ggplot() +
  # Full region
...
Single variable marginals indicate peaks in the pKas/pH-correction and a threshold for the concentration, regardless of the condition.
The marginals for low energy and viable performance (competitive vs MEA) are similar, indicating the importance of energy demands to being competitive.

```{r Plots: Importance Ranking - Natural Variables}
# Plot results
...
```{r Importance Ranking Function - Relative variables}
marginals1D.rel = function(E.cutof, F.cutof, mod.flux.a, mod.ener.a, mod.flux.b, mod.ener.b){
  # Cutoff values should be converted based on the model conversions to log units
  E.cutof = log10(E.cutof)
  # F.cutof = F.cutof
  # Constants
  ...
  # Dataframe storage
  post.optim = data.frame(pka1 = seq(from = pka1.rng[1], to = pka1.rng[2], length.out = resolution),
                          pka2 = seq(from = pka2.rng[1], to = pka2.rng[2], 
                                     length.out = resolution),
                          logA = seq(from = logA.rng[1], to = logA.rng[2], length.out = resolution),
                          Na.A = seq(from = Na.A.rng[1], to = Na.A.rng[2], 
                                     length.out = resolution),
          # Combined acid and base for importance ranking
          p.pka1 = NaN, p.pka2 = NaN, p.logA = NaN, p.Na.A = NaN, # Probability acceptance median
          s.pka1 = NaN, s.pka2 = NaN, s.logA = NaN, s.Na.A = NaN, # variance for importance ranking
...
# Take the marginals
import.rank.rel = function(post.optim, post.opt.typ){
  # Calculating the rankings
  # Higher variance = other variables are more important (inverse of variance = more important control variable)
  # Greater range of probabilities = more important
  import = data.frame(rng = c(diff(range(post.optim$p.pka1)), diff(range(post.optim$p.pka2)),
...
```{r Importance Ranking Results - relative variables}
# Run function: 1D marginals
margin.reg1 = marginals1D.rel(E.cutof = reg.full$max.ener, F.cutof = reg.full$min.flux,
            mod.flux.a.rel, mod.ener.a.rel, mod.flux.b.rel, mod.ener.b.rel)
margin.reg2 = marginals1D.rel(E.cutof = reg.flux$max.ener, F.cutof = reg.flux$min.flux,
            mod.flux.a.rel, mod.ener.a, mod.flux.b.rel, mod.ener.b.rel)
margin.reg3 = marginals1D.rel(E.cutof = reg.ener$max.ener, F.cutof = reg.ener$min.flux,
            mod.flux.a.rel, mod.ener.a.rel, mod.flux.b.rel, mod.ener.b.rel)
# Run function: importance rank
import.reg1 = import.rank.rel(post.optim = margin.reg1$var, post.opt.typ = margin.reg1$base)
import.reg2 = import.rank.rel(post.optim = margin.reg2$var, post.opt.typ = margin.reg2$base)
import.reg3 = import.rank.rel(post.optim = margin.reg3$var, post.opt.typ = margin.reg3$base)

# Store information so it does not have to be run again
margin.reg1$var$region = 'Full Viable Region'
margin.reg2$var$region = 'Maximum Flux'
margin.reg3$var$region = 'Minimum Energy'
write.csv(rbind(margin.reg1$var, margin.reg2$var, margin.reg3$var), '1DMarginals_Relative.csv')

margin.reg1$base$region = 'Full Viable Region'
margin.reg2$base$region = 'Maximum Flux'
margin.reg3$base$region = 'Minimum Energy'
write.csv(rbind(margin.reg1$base, margin.reg2$base, margin.reg3$base), 'AcidBaseMarginal_Relative.csv')

import.reg1$region = 'Full Viable Region'
import.reg2$region = 'Maximum Flux'
import.reg3$region = 'Minimum Energy'
write.csv(rbind(import.reg1, import.reg2, import.reg3), 'ImportanceRanking_Relative.csv')

```

```{r Reload relative variable data}
marg = read.csv('1DMarginals_Relative.csv')
base = read.csv('AcidBaseMarginal_Relative.csv')
impo = read.csv('ImportanceRanking_Relative.csv')
...
```{r 1D Marginals: relative variables}
ggplot() +
  # Full region
  geom_path(data = margin.reg1$var, mapping = aes(x = pka1, y = l.pka1, color = 'Full'), linetype = 2) +
  geom_path(data = margin.reg1$var, mapping = aes(x = pka1, y = h.pka1, color = 'Full'), linetype = 2) +
...
ggplot() +
  ...
  geom_path(data = margin.reg3$var, mapping = aes(x = pka2, y = h.pka2, color = 'Ener'), linetype = 2) +
  geom_path(data = margin.reg3$var, mapping = aes(x = pka2, y = p.pka2, color = 'Ener')) +
  labs(x = expression('p'*italic(K)['a,2']*' - p'*italic(K)['a,1']), y = 'Probability', color = '') +
  scale_color_manual(name = '',
                     labels = c('Full' = 'Viable', 'Flux' = 'High Flux (*2)', 'Ener' = 'Low Energy'),
                     values = c('Full' = 'black', 'Flux' = 'red', 'Ener' = 'blue'))
...
ggplot() +
  ...
  geom_path(data = margin.reg3$var, mapping = aes(x = 10^Na.A, y = h.Na.A, color = 'Ener'), linetype = 2) +
  geom_path(data = margin.reg3$var, mapping = aes(x = 10^Na.A, y = p.Na.A, color = 'Ener')) +
  labs(x = expression('log'[10]*' {Acid or Base} - log'[10]*' {Quinone}'), y = 'Probability', color = '') +
  scale_color_manual(name = '',
                     labels = c('Full' = 'Viable', 'Flux' = 'High Flux (*2)', 'Ener' = 'Low Energy'),
                     values = c('Full' = 'black', 'Flux' = 'red', 'Ener' = 'blue')) +
  scale_x_log10()
...
```{r Plots: Importance Ranking - Relative Variables}
# Plot results
ggplot(import.reg1[order(import.reg1$rank, decreasing = TRUE),]) +
  geom_col(mapping = aes(x = 1:nrow(import.reg1), y = rank/max(rank), fill = var)) +
  geom_errorbar(mapping = aes(x = 1:nrow(import.reg1), y = rank/max(rank), 
  ...
  labs(x = '', y = '', color = '', subtitle = 'Generally Acceptable Performance: Feature Importance') +
  scale_fill_discrete(labels = c('pka1' = expression('p'*italic(K)['a,1']),
    'pka2' = expression('p'*italic(K)['a,2']*' - p'*italic(K)['a,1']), 'logA' = '{Quinone}',
    'Na.A' = '{NaOH or HCl}/{Quinone}', 'Base' = 'Acid/Base'),
    breaks = import.reg1$var[order(import.reg1$rank, decreasing = TRUE)], name = '') +
  scale_x_discrete(labels = c()) +
  scale_y_continuous(breaks = c(0, 1), expand = expansion(mult = c(0, .1)),
                     labels = c('Least', 'Most'), name = 'Importance') +
...
In terms of relative variables, the ranking is:
* pKa1 (ie. median pKa)
* Quinone concentration
* Acid/Base choice
* pKa difference
* Ratio of acid/base and quinone concentration

Combining these two rankings, it shows that the relative difference between the two pKas and the ratio of pH correction species to the quinone concentration is largely unimportant.
These two relative variables are less important the the decision of whether to use an acid or base, whose probabilities and distribution were unchanged in the models.

The remainder of this analysis will solely be in terms of natural variables for ease of interpretability.
...
```{r Suggested Ranges: Functions - natural variables}
suggest.pka2 = function(E.cutof, F.cutof, mod.ener.a, mod.flux.a, mod.ener.b, mod.flux.b, resolution, MCsamp){
  ...
  for(i in 1:resolution){
    ...
    range.pka2$p.a[i] = mean(p.a); range.pka2$p.b[i] = mean(p.b)
    range.pka2$s.a[i] = sd(p.a);   range.pka2$s.b[i] = sd(p.b)
  }
  # Suggested ranges
  ...
  pka2.pk.wk.a = filter(pka2.wk, p.a == max(p.a))$pka2[1]
  pka2.pk.wk.b = filter(pka2.wk, p.b == max(p.b))$pka2[1]
  pka2.pk.st.a = filter(pka2.st, p.a == max(p.a))$pka2[1]
  pka2.pk.st.b = filter(pka2.st, p.b == max(p.b))$pka2[1]
  # Min values for acid and base
  pka2.lo.wk.a = filter(range.pka2, p.a == min(p.a))$pka2[1]
  pka2.lo.wk.b = filter(range.pka2, p.b == min(p.b))$pka2[1]
  pka2.lo.st.a = filter(range.pka2, p.a == min(p.a))$pka2[1]
  pka2.lo.st.b = filter(range.pka2, p.b == min(p.b))$pka2[1]

  # Also include the median as a possible option for the peak
  return(list(peak = data.frame(wk = pka2.rng.wk, st = pka2.rng.st, 
              ...
              marginal = data.frame(p = c(range.pka2$p.b, range.pka2$p.a),
                                    s = c(range.pka2$s.a, range.pka2$s.b),
                                    var = range.pka2$pka2, name = 'pka2',
                                    typ = c(rep('+Base', nrow(range.pka2)), rep('+Acid', nrow(range.pka2))) 
                                    )))
...
suggest.pka1 = function(E.cutof, F.cutof, mod.ener.a, mod.flux.a, mod.ener.b, mod.flux.b, range.pka2, resolution, MCsamp){
  ...
  for(i in 1:resolution){
    ...
    range.pka1.st$s.a[i] = sd(p.a); range.pka1.st$s.b[i] = sd(p.b)
  ...
  # Peak values.
  pka1.pk.wk.a = filter(pka1.wk, p.a == max(p.a))$pka1[1]
  pka1.pk.wk.b = filter(pka1.wk, p.b == max(p.b))$pka1[1]
  pka1.pk.st.a = filter(pka1.st, p.a == max(p.a))$pka1[1]
  pka1.pk.st.b = filter(pka1.st, p.b == max(p.b))$pka1[1]
  # Minimum values
  pka1.lo.wk.a = filter(range.pka1.wk, p.a == min(p.a))$pka1[1]
  pka1.lo.wk.b = filter(range.pka1.wk, p.b == min(p.b))$pka1[1]
  pka1.lo.st.a = filter(range.pka1.st, p.a == min(p.a))$pka1[1]
  pka1.lo.st.b = filter(range.pka1.st, p.b == min(p.b))$pka1[1]
  
  return(list(peak = data.frame(wk = pka1.rng.wk, st = pka1.rng.st,
              ...
              marginal = data.frame(p = c(range.pka1.st$p.b, range.pka1.st$p.a),
                                    s = c(range.pka1.st$s.b, range.pka1.st$s.a),
                                    var = range.pka1.st$pka1, name = 'pka1',
                                    typ = c(rep('+Base', nrow(range.pka1.st)), rep('+Acid', nrow(range.pka1.st)))) 
                                    ))
}
...
suggest.logA = function(E.cutof, F.cutof, mod.ener.a, mod.flux.a, mod.ener.b, mod.flux.b, 
  ...
  for(i in 1:resolution){
    ...
    range.logA$s.a[i] = sd(p.a); range.logA$s.b[i] = sd(p.b)
  ...
  # Peaks
  logA.pk.wk.a = filter(logA.wk, p.a.wk == max(p.a.wk))$logA[1]
  logA.pk.wk.b = filter(logA.wk, p.b.wk == max(p.b.wk))$logA[1]
  logA.pk.st.a = filter(logA.st, p.a.st == max(p.a.st))$logA[1]
  logA.pk.st.b = filter(logA.st, p.b.st == max(p.b.st))$logA[1]
  # Peaks
  logA.lo.wk.a = filter(range.logA, p.a.wk == min(p.a.wk))$logA[1]
  logA.lo.wk.b = filter(range.logA, p.b.wk == min(p.b.wk))$logA[1]
  logA.lo.st.a = filter(range.logA, p.a.st == min(p.a.st))$logA[1]
  logA.lo.st.b = filter(range.logA, p.b.st == min(p.b.st))$logA[1]
  
  return(list(peak = data.frame(wk = logA.rng.wk, st = logA.rng.st, 
              ...
              marginal = data.frame(p = c(range.logA$p.b.st, range.logA$p.a.st),
                                    s = c(range.logA$s.b, range.logA$s.a),
                                    var = range.logA$logA, name = 'logA',
                                    typ = c(rep('+Base', nrow(range.logA)), rep('+Acid', nrow(range.logA))) 
                                    )))
}
...
suggest.Na.A = function(E.cutof, F.cutof, mod.ener.a, mod.flux.a, mod.ener.b, mod.flux.b, 
  ...
  for(i in 1:resolution){
    ...
    range.Na.A$s.a[i] = sd(p.a);    range.Na.A$s.b[i] = sd(p.b)
    # Weak suggestion
  ...
  # Peak values
  Na.A.pk.wk.a = filter(Na.A.wk, p.Na.A.a.wk == max(p.Na.A.a.wk))$Na.A.wk[1]
  Na.A.pk.wk.b = filter(Na.A.wk, p.Na.A.b.wk == max(p.Na.A.b.wk))$Na.A.wk[1]
  Na.A.pk.st.a = filter(Na.A.st, p.Na.A.a.st == max(p.Na.A.a.st))$Na.A.st[1]
  Na.A.pk.st.b = filter(Na.A.st, p.Na.A.b.st == max(p.Na.A.b.st))$Na.A.st[1]
  # Minima values
  Na.A.lo.wk.a = filter(range.Na.A, p.Na.A.a.wk == min(p.Na.A.a.wk))$Na.A.wk[1]
  Na.A.lo.wk.b = filter(range.Na.A, p.Na.A.b.wk == min(p.Na.A.b.wk))$Na.A.wk[1]
  Na.A.lo.st.a = filter(range.Na.A, p.Na.A.a.st == min(p.Na.A.a.st))$Na.A.st[1]
  Na.A.lo.st.b = filter(range.Na.A, p.Na.A.b.st == min(p.Na.A.b.st))$Na.A.st[1]

  return(list(peak = data.frame(wk = Na.A.rng.wk, st = Na.A.rng.st,
              ...
              marginal = data.frame(p = c(range.Na.A$p.Na.A.b.st, range.Na.A$p.Na.A.a.st),
                                    s = c(range.Na.A$s.b, range.Na.A$s.a),
                                    var = range.Na.A$Na.A.st, name = 'logNa.A',
                                    typ = c(rep('+Base', nrow(range.Na.A)), rep('+Acid', nrow(range.Na.A))) 
                                    )))
}
...
suggest.ranges = function(E.cut, F.cut, mod.ener.a, mod.flux.a, mod.ener.b, mod.flux.b){
  ...
  region.pka1 = suggest.pka1(E.cutof = E.cut, F.cutof = F.cut,
               mod.ener.a = mod.ener.a, mod.flux.a = mod.flux.a,
               mod.ener.b = mod.ener.b, mod.flux.b = mod.flux.b,
               range.pka2 = region.pka2$peak,
               resolution = resolution, MCsamp = MCsamp)
  region.logA = suggest.logA(E.cutof = E.cut, F.cutof = F.cut,
               mod.ener.a = mod.ener.a, mod.flux.a = mod.flux.a,
               mod.ener.b = mod.ener.b, mod.flux.b = mod.flux.b,
               range.pka2 = region.pka2$peak, range.pka1 = region.pka1$peak,
               resolution = resolution, MCsamp = MCsamp)
  region.Na.A = suggest.Na.A(E.cutof = E.cut, F.cutof = F.cut,
               mod.ener.a = mod.ener.a, mod.flux.a = mod.flux.a,
               mod.ener.b = mod.ener.b, mod.flux.b = mod.flux.b,
               range.pka2 = region.pka2$peak, range.pka1 = region.pka1$peak, range.logA = region.logA$peak,
               resolution = resolution, MCsamp = MCsamp)
  return(list(pka2 = region.pka2$peak, pka1 = region.pka1$peak, 
              logA = region.logA$peak, Na.A = region.Na.A$peak,
              marg = rbind(region.pka2$marginal, region.pka1$marginal, 
                           region.logA$marginal, region.Na.A$marginal)))
}
```
...
# Save ranges and marginals
write.csv(x = reg.full.ranges[!names(reg.full.ranges) %in% 'marg'], file = 'AcceptRanges.csv')
write.csv(x = reg.flux.ranges[!names(reg.flux.ranges) %in% 'marg'], file = 'MaxFlxRanges.csv')
write.csv(x = reg.ener.ranges[!names(reg.ener.ranges) %in% 'marg'], file = 'MinEngRanges.csv')

# Save marginals
write.csv(x = reg.full.ranges[names(reg.full.ranges) %in% 'marg'], file = 'AcceptCondMargin.csv')
write.csv(x = reg.flux.ranges[names(reg.flux.ranges) %in% 'marg'], file = 'MaxFlxCondMargin.csv')
write.csv(x = reg.ener.ranges[names(reg.ener.ranges) %in% 'marg'], file = 'MinEngCondMargin.csv')
...
The suggested ranges appear to agree with the distribution of sampled points.
Generally, high flux conditions require more specific characteristics, in agreement with the fact that this region is very small compared to the other two.
...
# Store for later
region.pk$region = c(rep('Viable', 4), rep('High Flux', 4), rep('Low Energy', 4))
region.pk$suggest = c(rep('weak', 2), rep('strong', 2))
write.csv(region.pk, 'SuggestedPoints.csv')
region.pk = region.pk[, !names(region.pk) %in% c('region', 'suggeset')]
```
...
The suggested peak points all fall squarely within the regions that they target, showing the utility of this method as a predictor.
In fact, the high flux suggestions were more Pareto optimal, and thus may provide a means for refining the Pareto front.
...
* Poor performance as defined empirically by: 
  pKa1 = 4.92, pka2 = 9.49, {Q} = 23 mM, {HX} = 6.6 mM
...
# pKa1 = 4.92, pka2 = 9.49, {Q} = 23 mM, {HX} = 6.6 mM
test.sys = data.frame(pka1 = c(region.pk$pka1[7],  7.35,   region.pk$pka1[9], 4.92),
                      pka2 = c(region.pk$pka2[7],   11.25, region.pk$pka2[9], 9.49),
                      logA = c(region.pk$logA[7],   0.5,   region.pk$logA[9], -1.63),
                      Na.A = c(region.pk$Na.A[7],   -1,    region.pk$Na.A[9], -2.18),
                      acid = c(region.pk$typ[7],   -1,     region.pk$typ[9], -1),
                      label = c('Global High Flux', 'Local High Flux', 'Low Energy', 'Bad Result') )
# # Target points based on the marginals - best, worst, and 1-variable exchanges
exch.sys = data.frame(pka1 = rep(reg.full.ranges$pka1.pk.wk[1], 5), 
                      pka2 = rep(reg.full.ranges$pka2.pk.wk[1], 5), 
                      logA = rep(reg.full.ranges$logA.pk.wk[1], 5),
                      Na.A = rep(reg.full.ranges$Na.A.pk.wk[1], 5), acid = rep(1, 5))
...
wt = 0.45 # Weighted average so it does not move too far; interested in relatively small perturbations
...
ggplot() +
  ...
  geom_point(data = filter(test.sys)[c(1, 3, 4, 9),], 
...
g.points = ggplot() +
  ...
  geom_segment(data = filter(test.sys, Energy.kJ.mol <= 100, Flux.mol.m2s >= 0)[-c(1:3, 9),], 
  ...
  geom_point(data = filter(test.sys, Energy.kJ.mol <= 100, Flux.mol.m2s >= 0)[-c(1:3, 9),], 
...
test.plt = test.sys[-c(1:3, 9),]
g.dist = ggplot(test.plt[order(test.plt$dist, decreasing = TRUE),]) +
  geom_col(mapping = aes(x = 1:nrow(test.plt), 
  ...
  scale_x_continuous(breaks = 1:nrow(test.plt),
...
The "bad point" suggested by marginals was too far from the other points such that interpreting the impact of each variable was hard to untangle.
Instead, we picked a point with moderate energy demand but low flux to contrast with the low energy-moderate flux and the high energy-high flux suggestions.

For the points of interest, we plot the entire cycle dynamics to attempt to distinguish effects of each variable.
...
plt.demo.E = function(test.sys, i){
  ...
  E.test = PCET.electrode(soln.path = test.data, pka1 = test.sys$pka1[i], pka2 = test.sys$pka2[i])
  # Plots
  col = 'gray'
...
plt.demo.state(test.sys = test.sys, i = 9)
```

Important cycles to export for formal plotting:
```{r Cycles to Export}
stage.rename = function(test.data){
  test.data$stage[test.data$stage == '1->2'] = 'Oxidative Acidification'
  test.data$stage[test.data$stage == '2->3'] = 'CO2 Degassing'
  test.data$stage[test.data$stage == '3->4'] = 'Reductive Regeneration'
  test.data$stage[test.data$stage == '4->1'] = 'CO2 Capture'
  return(test.data)
}
# High global flux
i = 1;
test.data = PCET.cycle(pka1 = test.sys$pka1[i], pka2 = test.sys$pka2[i],
                       ...
                       acid = test.sys$acid[i])
E.test = PCET.electrode(soln.path = test.data, pka1 = test.sys$pka1[i], pka2 = test.sys$pka2[i])
test.data = stage.rename(test.data)
E.test = stage.rename(E.test)
# Store
write.csv(test.data, 'CycleProcess_HighFlux.csv')
write.csv(E.test, 'CycleVoltage_HighFlux.csv')
...
E.test = stage.rename(E.test)
# Store
write.csv(test.data, 'CycleProcess_PeakPredict.csv')
write.csv(E.test, 'CycleVoltage_PeakPredict.csv')
...
E.test = stage.rename(E.test)
# Store
write.csv(test.data, 'CycleProcess_PoorPredict.csv')
write.csv(E.test, 'CycleVoltage_PoorPredict.csv')

# Export points of interest
write.csv(test.sys[c(1, 4, 9), !names(test.sys) %in% 'dist'], 'Cycle_PointsOfInterest.csv')
...
plt.demo.E(test.sys = test.sys, i = 9)
...
p.cutof = max(quinone.data$p.accept)*0.7
...
More than half of the candidates have at least a 50% likelihood of performing well, indicating that there are a number of known candidates that may work.
The distribution of known quinones (weighted by their probabilities) shows that quinones with pKa near the single variable marginal peaks are important to investigate in future studies.

Breakdown into function groups, organized by their electronic effect (donating/withdrawing)
...
Electron donating groups increase the probability of viability substantially, but withdrawing groups are not necessarily a detriment given their wide distribution.
