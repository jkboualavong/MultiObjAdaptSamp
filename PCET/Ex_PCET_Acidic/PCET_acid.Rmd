...
fill.sample.mod = function(GPar.data, input.name, output.name){
  ...
  nug = min(abs(GPar.data[, output.name]))*1e-2 # Uncertainty should be smaller than the values
  while(log10(pt90/pt10) <= log10(pt75/pt25)){
    mod.out = km(design = GPar.data[, input.name], response = GPar.data[, output.name], 
                 ...
                 nugget = min(c(1e-6, nug)), # Avoid eigenvalues of 0
                 )
...
It appears that the GP model for the flux predictions always seem to reach the upper limit of iterations for convergence, but a 50% training/test split shows that the error never exceeds 10%.
...
The Pareto front is very similar to that of base addition.
...
* Excess acid addition leads to poor performance. The maximum amount of acid is approximately 1-tenth of the amonut of quinone.
...
The local peak in the quinone concentration still exists at around 100 mM, but the higher concentrations are just as viable under acid addition.
...
```{r Combined dataset}
# Load acid data
GPar.all.acid = read.csv(file = 'GPar_90CapOpt_data.csv')
...
write.csv(GPar.all, file = '../PCET_Proceess/GPar_AcidBase_data.csv')
write.csv(GPar.front, file = '../PCET_Proceess/GPar_AcidBase_fnt.csv')
# For the puposes of plotting:
# If the overall pH correctant is very close to 0 (<10^-7), then treat as zero
GPar.all$typ[GPar.all$Na.A + GPar.all$logA < -7] = 'None'

sz = 2
ggplot(filter(GPar.all, Energy.kJ.mol < 150, Flux.mol.m2s > 0)) +
  geom_line(GPar.front, mapping = aes(x = Flux.mol.m2s, y = Energy.kJ.mol), color = 'black') +
  geom_point(mapping = aes(x = Flux.mol.m2s, y = Energy.kJ.mol, color = typ, shape = typ), size = sz) +
  labs(x = 'CO2 Flux (mol/m2/s)', y = 'Energy Demand (kJ/mol C)', 
       subtitle = 'CO2 Capture by PCET-based pH swing', color = 'pH Correction', shape = 'pH Correction') +
  theme_classic() + theme(legend.position = c(0.9, 0.9), 
                          legend.box.background = element_rect(color = 'black', linetype = 1))

```

The dataset shows that acidic conditions tend to outperform basic conditions with higher fluxes and lower energy demands, and both generally outperform negligible pH correction (additional acid or base less than 0.1 uM).
...
# Models
mod.flux = fill.sample.mod(GPar.data = GPar.nat, input.name = c('pka1', 'pka2', 'logA', 'Na.A', 'typ'),
                           output.name = 'Flux.mol.m2s')
mod.ener = fill.sample.ener(GPar.data = GPar.nat, 
...
```{r Optimal Capture Marginals Acid+Base Natural variables: 1D}
# Define the cutoff values
E.cutof = log10(40); #kJ/mol C, log units
F.cutof = 0.1*max(GPar.all$Flux.mol.m2s)
...
# Account for acid/base as a free variable for metrics. Using this to determine if acid or base conditions should be used.
fill.frame = data.frame(pka1 = runif(n = MCsamp, min = pka1.rng[1], max = pka1.rng[2]),
                        pka2 = runif(n = MCsamp, min = pka2.rng[1], max = pka2.rng[2]),
                        logA = runif(n = MCsamp, min = logA.rng[1], max = logA.rng[2]),
                        Na.A = runif(n = MCsamp, min = Na.A.rng[1], max = Na.A.rng[2]),
...
fill.frame$p.accept = (1 - pnorm(q = 0, mean = res.flux$mean - F.cutof, sd = res.flux$sd)) *
  pnorm(q = 0, mean = res.ener$mean - E.cutof, sd = res.ener$sd)
post.opt.typ = data.frame(typ = -1, p = mean(fill.frame$p.accept),
                          s = sd(fill.frame$p.accept),
                          h = quantile(fill.frame$p.accept, probs = upper),
                          l = quantile(fill.frame$p.accept, probs = lower))
...
fill.frame$p.accept = (1 - pnorm(q = 0, mean = res.flux$mean - F.cutof, sd = res.flux$sd)) *
  pnorm(q = 0, mean = res.ener$mean - E.cutof, sd = res.ener$sd)
post.opt.typ = rbind(post.opt.typ, 
                     data.frame(typ = +1, p = mean(fill.frame$p.accept),
                          s = sd(fill.frame$p.accept),
                          h = quantile(fill.frame$p.accept, probs = upper),
                          l = quantile(fill.frame$p.accept, probs = lower)))
...
```{r Optimal Capture Marginals Acid+Base Natural variables: 1D Plots}
# Acid vs basic conditions
ggplot(post.opt.typ)+
  geom_col(mapping = aes(x = typ, y = p, fill = as.factor(typ))) +
  geom_errorbar(mapping = aes(x = typ, y = p, ymin = l, ymax = h), width = 0.75) +
  scale_x_continuous(breaks = c(-1, 1), labels = c('Acid', 'Base')) +
  labs(x = '', y = 'P[Optimal]') +
  guides(fill = FALSE)
...
Comparing the probabilities of adding acid vs base, it appears that adding acid consistently shows a higher probability of being optimal. Further analysis will assume acid addition for simplicity.
Since it is a categorical variable, it is difficult to compare its importance; a rough estimate of this comparison can be made by finding the expectation value of the variance instead of the total variance.

```{r Acid+Base Natural Variables: Importance Ranking}
...
# Attempt to normalize the area estimates to allow for a comparison to the acid/base addition decision

import.all = data.frame(rank = c(diff(range(post.opt.typ$p)) / mean(post.opt.typ$s),
                                 import$rank*resolution/2),
                        var = c('Base', import$var))
ggplot(import.all[order(import.all$rank, decreasing = TRUE),]) +
  geom_col(mapping = aes(x = 1:nrow(import.all), y = rank/max(rank), fill = var)) +
  labs(x = '', y = '', color = '', subtitle = 'Feature Importance') +
  scale_fill_discrete(labels = c('pka1' = expression('p'*italic(K)['a,1']),
                                 'pka2' = expression('p'*italic(K)['a,2']), 
                                 'logA' = '{Quinone}',
                                 'Na.A' = '{NaOH}',
                                 'Base' = 'Acid/Base'),
                      breaks = import.all$var[order(import.all$rank, decreasing = TRUE)],
                      name = '') +
  scale_x_discrete(labels = c()) +
  scale_y_continuous(breaks = c(0, 1),
                     expand = expansion(mult = c(0, .1)),
...
Importance ranking suggests that the quinone characteristics (pKas and, to a lesser extent, the solubility limit) are more important than the operating condition decisions (concentrations of quinone and pH correction compound, as well as the decision of acid or base).
The acid/base decision has very low importance largely because the difference between acidic and basic conditions is small (about 2% probability difference).
Since the difference is so small, I will simply perform the remaining conditions on the suggested ranges assuming acidic conditions; it can be assumed that basic conditions will have roughly the same parameters.
...
for(i in 1:resolution){
  ...
  fill.frame$p.accept = (1 - pnorm(q = 0, mean = res.flux$mean - F.cutof, sd = res.flux$sd)) *
    pnorm(q = 0, mean = res.ener$mean - E.cutof, sd = res.ener$sd)
  range.pka2$p.pka2[i] = mean(fill.frame$p.accept)
}

# Plot the range to determine how to describe the optimal range
ggplot(range.pka2) +
  geom_path(mapping = aes(x = pka2, y = p.pka2)) +
  geom_hline(yintercept = 0.5*diff(range(range.pka2$p.pka2))+min(range.pka2$p.pka2), linetype = 2, color = 'red') +
  geom_hline(yintercept = 0.75*diff(range(range.pka2$p.pka2))+min(range.pka2$p.pka2), linetype = 2) +
  labs(x = expression('p'*italic(K)['a,2']), y = 'P[Optimal]')
...
print('Second pKa:')
pka2.rng.wk = range(filter(range.pka2, p.pka2 > 0.5*diff(range(p.pka2))+min(p.pka2))$pka2)
pka2.rng.st = range(filter(range.pka2, p.pka2 > 0.75*diff(range(p.pka2))+min(p.pka2))$pka2)
print('Weak range: Between')
print(round(pka2.rng.wk, 2))
print('Strong range: Between')
print(round(pka2.rng.st, 2))

```

Range for the 2nd pKa:
Weak suggestion:   8.53 ~ 19.00
Strong suggestion: 9.21 ~ 17.45

More generally: greater than the lower bound. The dip in the probability near pKa = 15 is inconsistent with repeated measures.
...
for(i in 1:resolution){
  # Strong suggestion
  pka2.testrng = c(max(pka2.rng.st[1], range.pka1.st$pka2[i] - pka2.rng[2]),
                   min(pka2.rng.st[2], range.pka1.st$pka2[i] - pka2.rng[1]))
  ...
  fill.frame$p.accept = (1 - pnorm(q = 0, mean = res.flux$mean - F.cutof, sd = res.flux$sd)) *
    pnorm(q = 0, mean = res.ener$mean - E.cutof, sd = res.ener$sd)
  range.pka1.st$p.pka1[i] = mean(fill.frame$p.accept)
  
  # Weak suggestion
  pka2.testrng = c(max(pka2.rng.wk[1], range.pka1.wk$pka2[i] - pka2.rng[2]),
                   min(pka2.rng.wk[2], range.pka1.wk$pka2[i] - pka2.rng[1]))
  ...
  fill.frame$p.accept = (1 - pnorm(q = 0, mean = res.flux$mean - F.cutof, sd = res.flux$sd)) *
    pnorm(q = 0, mean = res.ener$mean - E.cutof, sd = res.ener$sd)
  range.pka1.wk$p.pka1[i] = mean(fill.frame$p.accept)
}

# Plot the probabilities to determine how to describe the optimal range
ggplot() +
  geom_path(data = range.pka1.wk, mapping = aes(x = pka1, y = p.pka1, color = 'weak')) +
  geom_hline(yintercept = 0.5*diff(range(range.pka1.wk$p.pka1))+min(range.pka1.wk$p.pka1), 
             color = 'red', linetype = 2) +
  geom_path(data = range.pka1.st, mapping = aes(x = pka1, y = p.pka1, color = 'strong')) +
  geom_hline(yintercept = 0.75*diff(range(range.pka1.st$p.pka1))+min(range.pka1.st$p.pka1),
             color = 'black', linetype = 2) +
  labs(x = expression('p'*italic(K)['a,1']), y = expression('P[Optimal | p'*italic(K)['a,2']*']')) +
  scale_color_manual(labels = c('weak' = 'Weak', 'strong' = 'Strong'),
                     name = 'Suggestion',
                     values = c('weak' = 'red', 'strong' = 'black'))

# Weak range: between 7~10 and above 12. However, the tailing effect of being above 12 is barely over the cutoff, so will be omitted for simplicity.
pka1.rng.wk = range(filter(range.pka1.wk, p.pka1 > 0.5*diff(range(p.pka1)) +min(p.pka1))$pka1)
pka1.rng.st = range(filter(range.pka1.st, p.pka1 > 0.75*diff(range(p.pka1))+min(p.pka1))$pka1)
...
Weak suggestion:   4.29 ~ 11.17
Strong suggestion: 5.99 ~ 10.34

The quinone and acid concentrations have similar importance, and thus will be assessed separately first.
...
# Appears to be a lower bound. There is some oscillation with a dip in the probability at around 1 M, but this is likely uncertainty in the marginalization or in the model construction.
...
print('Quinone Concentration (M):')
print('Weak range: Between')
signif(10^logA.rng.wk, 2)
print('Strong range: Between')
signif(10^logA.rng.st, 2)


```


```{r Optimal Capture Marginals Natural variables: Suggested Range Acid concentration}
# Set up the marginalization, higher resolution than the single variable marginals for plotting
resolution = 100; MCsamp = 1000
pka2.rng = c(0, 5.5); logA.rng = c(-2, 0.5); Na.A.rng = c(-7, 0.7)
...
ggplot() +
  ...
  geom_hline(yintercept = 0.75*diff(range(range.Na.A$p.Na.A.st))+min(range.Na.A$p.Na.A.st),
             color = 'black', linetype = 2) +
  labs(x = '{Acid}', y = expression('P[Optimal | p'*italic(K)['a,1']*', p'*italic(K)['a,2']*']')) +
  scale_color_manual(labels = c('weak' = 'Weak', 'strong' = 'Strong'),
                     name = 'Suggestion',
                     values = c('weak' = 'red', 'strong' = 'black')) +
  scale_x_log10()

# Optimal amount of acid to add
Na.A.rng.wk = range(filter(range.Na.A, p.Na.A.wk > 0.5*diff(range(p.Na.A.wk)) +min(p.Na.A.wk))$Na.A.wk)
Na.A.rng.st = range(filter(range.Na.A, p.Na.A.st > 0.75*diff(range(p.Na.A.st))+min(p.Na.A.st))$Na.A.st)

print('Acid concentration:')
print('Weak range: Between')
signif(10^Na.A.rng.wk, 2)
print('Strong range: Between')
signif(10^Na.A.rng.st, 2)


```

Approximate ranges (may differ from results above slightly due to differences in the model and Monte Carlo sampling error)

From most important to least important. Parenthesis indicate the upper limit of interpolation.

Range for the 2nd pKa:
Weak suggestion:   8.70 + (19)
Strong suggestion: 9.38 ~ 15.91

Range for the 1st pKa:
Weak suggestion:   5.80 ~ 12.36
Strong suggestion: 6.70 ~ 10.49

Quinone concentration
Weak suggestion:    25 mM + (3.2 M)
Strong suggestion: 180 mM + (3.2 M)

Acid concentration (M)
Weak suggestion:      8 nM ~ 0.26 M
Strong suggestion: 0.22 uM ~ 170 mM
...
range(filter(quinone.data, p.accept > max(p.accept)*0.9)$Pka.1)
range(filter(quinone.data, p.accept > max(p.accept)*0.9)$Pka.2)
...
g1 = ggplot() +
  ...
  geom_density(data = filter(quinone.data, p.accept > max(p.accept)*0.9), 
...
Further analysis on substituent effects may parse this information.
