...
* energy demand < 40 kJ/mol C to be competitive with temperature-swing processes
* flux > 1/10th of the maximum flux to be as fast as possible

The Pareto front has a slight concavity at around 0.22 mmol/m2*s where the energy demand increases but the flux remains constant.
...
reg.full = data.frame(min.flux = 0.1*max(GPar.front$Flux.mol.m2s),
...
reg.flux = data.frame(min.flux = 0.22e-3, max.flux = max(GPar.front$Flux.mol.m2s),
                     min.ener = min(GPar.front$Energy.kJ.mol),
                     max.ener = 40)
# Region 3: the bottom section of low energy but moderate flux
reg.ener = data.frame(min.flux = 0.1*max(GPar.front$Flux.mol.m2s),
                     max.flux = max(GPar.front$Flux.mol.m2s), 
                     min.ener = min(GPar.front$Energy.kJ.mol),
...
ggplot() +
  ...
  # High flux region
  geom_path(data = margin.reg2$var, mapping = aes(x = pka1, y = l.pka1, color = 'Flux'), linetype = 2) +
  geom_path(data = margin.reg2$var, mapping = aes(x = pka1, y = h.pka1, color = 'Flux'), linetype = 2) +
  geom_path(data = margin.reg2$var, mapping = aes(x = pka1, y = p.pka1, color = 'Flux')) +
  # Low Energy region
  ...
  scale_color_manual(name = '',
                     labels = c('Full' = 'Viable', 'Flux' = 'High Flux', 'Ener' = 'Low Energy'),
...
ggplot() +
  ...
  # High flux region
  geom_path(data = margin.reg2$var, mapping = aes(x = pka2, y = l.pka2, color = 'Flux'), linetype = 2) +
  geom_path(data = margin.reg2$var, mapping = aes(x = pka2, y = h.pka2, color = 'Flux'), linetype = 2) +
  geom_path(data = margin.reg2$var, mapping = aes(x = pka2, y = p.pka2, color = 'Flux')) +
  # Low Energy region
...
ggplot() +
  # Full region
  geom_path(data = margin.reg1$var, mapping = aes(x = logA, y = l.logA, color = 'Full'), linetype = 2) +
  geom_path(data = margin.reg1$var, mapping = aes(x = logA, y = h.logA, color = 'Full'), linetype = 2) +
  geom_path(data = margin.reg1$var, mapping = aes(x = logA, y = p.logA, color = 'Full')) +
  # High flux region
  geom_path(data = margin.reg2$var, mapping = aes(x = logA, y = l.logA, color = 'Flux'), linetype = 2) +
  geom_path(data = margin.reg2$var, mapping = aes(x = logA, y = h.logA, color = 'Flux'), linetype = 2) +
  geom_path(data = margin.reg2$var, mapping = aes(x = logA, y = p.logA, color = 'Flux')) +
  # Low Energy region
  geom_path(data = margin.reg3$var, mapping = aes(x = logA, y = l.logA, color = 'Ener'), linetype = 2) +
  geom_path(data = margin.reg3$var, mapping = aes(x = logA, y = h.logA, color = 'Ener'), linetype = 2) +
  geom_path(data = margin.reg3$var, mapping = aes(x = logA, y = p.logA, color = 'Ener')) +
...
ggplot() +
  # Full region
  geom_path(data = margin.reg1$var, mapping = aes(x = Na.A, y = l.Na.A, color = 'Full'), linetype = 2) +
  geom_path(data = margin.reg1$var, mapping = aes(x = Na.A, y = h.Na.A, color = 'Full'), linetype = 2) +
  geom_path(data = margin.reg1$var, mapping = aes(x = Na.A, y = p.Na.A, color = 'Full')) +
  # High flux region
  geom_path(data = margin.reg2$var, mapping = aes(x = Na.A, y = l.Na.A, color = 'Flux'), linetype = 2) +
  geom_path(data = margin.reg2$var, mapping = aes(x = Na.A, y = h.Na.A, color = 'Flux'), linetype = 2) +
  geom_path(data = margin.reg2$var, mapping = aes(x = Na.A, y = p.Na.A, color = 'Flux')) +
  # Low Energy region
  geom_path(data = margin.reg3$var, mapping = aes(x = Na.A, y = l.Na.A, color = 'Ener'), linetype = 2) +
  geom_path(data = margin.reg3$var, mapping = aes(x = Na.A, y = h.Na.A, color = 'Ener'), linetype = 2) +
  geom_path(data = margin.reg3$var, mapping = aes(x = Na.A, y = p.Na.A, color = 'Ener')) +
...
exch.sys = data.frame(pka1 = rep(reg.full.ranges$pka1.pk.st[1], 5), 
                      pka2 = rep(reg.full.ranges$pka2.pk.st[1], 5), 
                      logA = rep(reg.full.ranges$logA.pk.st[1], 5),
                      Na = rep(reg.full.ranges$Na.A.pk.st[1], 5), acid = rep(1, 5))
poor = c(reg.full.ranges$pka1.lo.st[1], reg.full.ranges$pka2.lo.st[1], 
         reg.full.ranges$logA.lo.st[1], reg.full.ranges$Na.A.lo.st[1])
wt = 0 # Weighted average so it does not move too far for plotting purposes. Set to 0 as a check
...
ggplot() +
  ...
  geom_line(GPar.front, mapping = aes(x = Flux.mol.m2s*1e3, y = Energy.kJ.mol), color = 'black') +
  geom_hline(yintercept = 40, color = 'black', linetype = 2) +
  ...
  geom_point(data = filter(test.sys, Energy.kJ.mol < 50, Flux.mol.m2s > 0)[c(1:3, 8),], 
             mapping = aes(x = Flux.mol.m2s*1e3, y = Energy.kJ.mol, color = label), 
             shape = 19, size = 3, stroke = 2) +
  labs(x = expression('CO2 Flux (mmol/m'^2*' s)'), y = expression('Energy Demand (kJ'['e']*'/mol C)'), 
