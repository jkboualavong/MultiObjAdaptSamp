---
title: "Proof of concept: CO2 capture with PCET-based pH swings, Base additions"
...
## Parameter space
As established in the preparatory calculations (/../Prep), data for restricting the quinone features is based on results from Huynh et al. J Am Chem Soc. 2016 December 14; 138(49): 15903–15910. doi:10.1021/jacs.6b05797.
...
It is clear that the flux can be very low and the energy demand can extend to very high numbers, such that visualizing the Pareto frontier with any resolution requires filtering out points that are too far from it.
...
This will be done by making a GP model of the kinetic rate, adaptively sampling points near the flux of 0 (signifying 90% capture), then marginalizating the probability that the result has positive flux.
...
```{r 90% Capture Refinement Iterations, warning=FALSE, message=FALSE}
# Load data
GPar.all = read.csv(file = 'GPar_all_data.csv')
# Remove indices
...
# Repeat for a maximum of 100 iterations, or until the fitness drops below 1/1000 of the starting fitness, 
...
In addition to refining knowledge of the boundary where the flux is zero (90% capture), I show the distribution of "good" points (above 10^-4 mol/m2/s flux) and "bad" points (negative flux).
...
These distributions should be interpreted with caution, though, since they are biased by the non-uniform sampling of the system.
...
for(i in 1:resolution){
  ...
  fill.frame$p.accept = 1- pnorm(q = 0, mean = res$mean - 1e-5, sd = res$sd)
...
* High pKas, particularly pKas with larger difference between them, are more likely to have high fluxes.
* Higher concentrations of quinone have nearly guaranteed probabilities of capturing CO2.
* The amount of base that is added is relatively inconsequential, as the probability is nearly flat.

While these cannot be used to remove areas of the search space from sampling because there are no points that have an effectively 0 probability mean, it does give some indication of what to consider when comparing options.
...
What is evident from these 2D partial component probability plots is that there are regions where the probability is very low, but they are not clearly delineated by single variables.
The only case where there might be a cutoff is the concentration of quinone, where low (<0.01 M) concentrations lead to generally low probabilities of capture, but this region is so small that removing it is not likely to make a large impact on the subsequent sampling points.

Insights from these 2D plots:

* Lower pKa2 values have the potential to lead to worse outcomes, particularly if it is below the carbonic acid pKa.
* Generally speaking, the concentration of additional acid or base has next to no impact on the flux. There is a slight trend towards moderate concentrations, but it is difficult to tell if that is caused by the edge effects (less sampling).
* The quinone concentration generally increases the probability of capture. There is large variability when the concentration of quinone and base are both high.
...
ggplot() +
  geom_vline(xintercept = 0.22e-3, linetype = 2) +
...
```{r Optimal Capture Marginals: Functions}
...
Not only did this sampling process help focus the boundaries, it also helped fill in parts of the Pareto frontier.
This may be in part due to the emphasis on exploration, but also be because the Pareto front itself is not that far from the selection criteria that were set.
...
* There is a minimum pKa2 for capture at around 7.5-9.5. Below this point, capture is very unlikely, and though above this point, there is large variance.
* There is an optimal range for the concentration of quinone. Interestingly, this is a discrete optimum and not a monotonic increase. Above the mid-range of concentrations, the variance appears to be higher. It is likely that increasing the amount of viable NaOH would broaden the range, but the maximum concentration of NaOH in these calculations (10^1.2, or about 15 M) is above the solubility limit of NaOH.
...
* The 1D marginals confirm the optimum in pKa2 that was observed in the 2D marginalization, but it also emphasizes the impact of pKa1 that was obscured in the 2D marginal.
...
region.flux = data.frame(flux.max = max(GPar.front$Flux.mol.m2s), flux.min = 2.2e-3,
...
fill.sample.sub.flux = function(x, model.flux, model.ener){
  ...
  F.cutof = 2.2e-3 # mol/m2s, flux at which there is a concavity in the Pareto front
...
ggplot() +
  ...
  guides(colour = guide_legend(override.aes = list(shape = 15, size = 2, alpha = c(0.5, 0.5, 1),
                                                   fill = NA, linetype = c(0, 0, 0)))) +
  labs(x = 'CO2 Flux (mol/m2/s)', y = 'Energy (kJ/mol C)', subtitle = 'Subregions after refinement')
