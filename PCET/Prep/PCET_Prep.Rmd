...
The work assumes that the redox molecule is a quinone (or, more generally, a 2 electron/2 proton PCET process with the possibility of deprotonation in the reduced state), and restricts the search to only combinations of properties that a quinone is likely to have based on available data.

Specifically, this particular notebook determines the following preparator calculations:
...
## Parameter space
Data for restricting the quinone features is based on data from Huynh et al. J Am Chem Soc. 2016 December 14; 138(49): 15903–15910. doi:10.1021/jacs.6b05797.
...
Similarly, the reduction potential of oxygen evolution will be at its lowest at the highest pH, which would be around the hydroquinone's buffer region. As a conservative estimate, assume it is the higher of its pKas + 1.

Practically, the quinone electrochemistry has much faster kinetics than either gas evolution reaction in the absence of a specific water-splitting catalyst, so using these potentials as a bound should limit the gas evolution reactions sufficiently.
...
There is a Bronsted-like relationship between the reduction potential and the reaction rate of the deprotonated hydroquinone nucleophilically attacking the CO2 [Simpson & Durand 1990, doi: 10.1016/0013-4686(90)85012-C], but this is not likely to be the dominant mechanism because the reaction with $OH^-$ is approximately 10 to 100 times faster in water aqueous environments.

Because the pKas are co-correlated, to make sampling easier, I convert the system into a free variable pKa and the difference between them. This conversion leads to a rectangular input space, rather than a parallelogram shape.
...
The variance alone show very little difference between the choices, which pKa1 being the free variable having a slightly better response.
...
* It is very unlikely that the second pKa is lower than the first. Only one measurement violates this criteria (ie. is below the lower of the red dotted lines) within measurement error.
...
The objective function for energy demand solves the set of equilibrium equations across the range of charge and gas transfers based on four variables defining the state of the system: 
...
Charge balance of the anolyte and catholyte are assumed to be the result of the background electrolyte traveling across an ion selective membrane.
...
The function can accommodate feed CO2 gases of any partial pressure, but this study is interested in application to coal flue gas (15v% feed to 1.5v% lean).

### Explicit and derived functions
...
Derived functions use the information from the explicit functions to determine relevant information for determining the energy demand and CO2 flux but are not themselves the energy demand or flux.
...
These variables are included because other compounds have been proposed to capture CO2 primarily through that mechanism, and thus they could also be studied with this set of functions.
...
Further tuning of the exact value of L is based on the minimum energy demand for the specific lean gas minimum.

An initial test of the simplest weighting function using only the ratio of the work of separations as the maximum weight:
```{r}
...
Simple weighted energy is not enough of a penalty; the minimum energy for systems that do not capture >90% of the CO2 are still showing lower energy demands (very high lean gas pressures are ~0 kJ/mol C)
...
While better when using the complete expression, using the complete weight expression does not completely correct the fact that the minimum in the energy occurs at a condition when less than 90% of the CO2 was removed.
Amplifying an additional multiplicative factor to amplify the weight:
...
Multiplying the logarithmic term by a factor of 25 appears to be sufficiently large to adjust the energies without overshooting the weight and negatively impacting the energy demands of conditions that do capture enough CO2.
...
Empirically, 30wt% MEA solutions absorb approximately 0.5~0.6 mol CO2/mol MEA at 0.13~0.15 atm [Puxty 2009], although at 10% of that pressure, the loading is bounded by a pinch point at around 0.225 mol/mol [Richner 2015].

$Ha = \sqrt{ D k C} / k_L$
$E = 1 + D*c_{MEA} / (z * D_{CO_2} * C_{CO_2})$
...
flux = sqrt(D.CO2 * k * C.MEA) * (C.CO2) * (1/1e3) * (100^2) 
# Since concentration was in mol/L, converting to mol/m^2*s
