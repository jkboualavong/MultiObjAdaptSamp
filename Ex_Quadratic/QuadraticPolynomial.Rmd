...
# Description
This notebook showcases the method below for finding acceptable suboptimal performance in the quadratic bi-objective function from Marler and Arora 2010.
The simple quadratic objective functions allow for easy conceptualization of the procedure to explore how and why it works.
...
n.obj = function(GPar.data, GPar.front){
  ...
  # In the event that the optimum is worse:
...
ggplot() +
  ...
  theme_classic() + theme(legend.position = c(0.9, 0.85), legend.background = element_rect(fill = 'white')) +
...
gplt.start = ggplot() +
  ...
  theme_classic() + theme(legend.position = c(0.85, 0.2), legend.background = element_rect(fill = 'white', linetype = 1, color = 'black'), legend.title = element_blank())
...
gplt.GPar = ggplot() +
  ...
  labs(x = "Function 1", y = "Function 2", color = "") +  
  scale_color_discrete(labels = c('Starting Data', 'Pareto Search'), breaks = c(FALSE, TRUE)) +
  theme_classic() + theme(legend.position = c(0.85, 0.3), legend.background = element_rect(fill = 'white', linetype = 1, color = 'black'), legend.title = element_blank())
...
# Calculate the fraction of the search space that is uncertain
start.perc = rep(0, nrow(fine.grid))
start.perc[fine.grid$dist.p1 >= 0.25 & fine.grid$dist.p1 <= 0.75] = 1
start.perc = mean(start.perc)
...
A typical stop criteria is based on the hypervolume fraction that is uncertain, i.e. an acceptance probability between 0.25 and 0.75. The fraction of the total input hypervolume space should be less than 0.5% of the total hypervolume space. This can be easily approximated by Monte Carlo sampling from the space and taking the fraction of points that are uncertain.

```{r Iteration loop: Accept distance less than 1 percent uncertainty cutoff, message=FALSE, warning=FALSE}
budget = 20
next.perc = start.perc
while(next.perc > 0.005 & max(GPar.all$order) < budget+Pareto.budget){
  ...
  # Collect samples for the Monte Carlo. Sample >10^(2N) points for N dimensions
  npt = 3000
  samp = data.frame(x1 = runif(n = npt, min = min(x1.rng), max = max(x1.rng)), 
                    x2 = runif(n = npt, min = min(x2.rng), max = max(x2.rng)))
  res = predict(object = mod.dist, newdata = samp, type = 'UK')
  samp$dist.p1 = pnorm(q = 0, mean = res$mean - 1, sd = res$sd)
  next.perc = rep(0, nrow(samp))
  next.perc[samp$dist.p1 >= 0.25 & samp$dist.p1 <= 0.75] = 1
  next.perc = mean(next.perc)
...
# rm(budget, n.samples, res, samp, next.perc)
```
...
This method has the problem of being biased by the sample points, rather than describing the model's output, which is the more important metric.
It is also a regression description, and therefore does not indicate any information related to whether it meets or fails the selection criteria.
...
The 25th and 75th percentiles were selected because they provide a good approximation of the variance without picking up noise due to sampling or being saturated by the bounds of probability at 0 or 1.
As a result, there is more likely to be a distinction between different variables, but it is also large enough to detect.
These percentiles can be manipulated as needed to avoid saturation depending on the problem, so long as they are the same for all variables.
...
ggplot() +
  ...
  geom_polygon(data = data.frame(x = c(Infer.x1$x, rev(Infer.x1$x)),
                              y = c(Infer.x1$p25, rev(Infer.x1$p75))),
            mapping = aes(x = x, y = y, fill = 'x1'), alpha = 0.25) +
  geom_polygon(data = data.frame(x = c(Infer.x2$x, rev(Infer.x2$x)),
                              y = c(Infer.x2$p25, rev(Infer.x2$p75))),
            mapping = aes(x = x, y = y, fill = 'x2'), alpha = 0.25) +
  ...
  scale_color_manual(name = 'Variable', values = c('x1' = 'red', 'x2' = 'blue'), 
                    labels = c('x1' = expression('x'[1]), 'x2' = expression('x'[2]))) +
  scale_fill_manual(name = 'Variable', values = c('x1' = 'red', 'x2' = 'blue'), 
                    labels = c('x1' = expression('x'[1]), 'x2' = expression('x'[2]))) +
...
ggplot(data.frame(import = c(resolution/sum(Infer.x1$p75 - Infer.x1$p25), 
  ...
  scale_fill_manual(values = c('purple3', 'orange'))

# Remove the percentiles for later
Infer.x1 = Infer.x1[,!names(Infer.x1) %in% c('p75', 'p25')]
Infer.x2 = Infer.x2[,!names(Infer.x2) %in% c('p75', 'p25')]
```

Consistent with the correlation coefficients, the probability of acceptance when only $x_1$ has a higher and sharper peak than $x_2$, but in this case, it shows the importance for a specific goal rather than the general trends.
This becomes more important when considering other selection criteria (below).
...
In this case, due to symmetry, there is similarity between x1 and x2's behavior as single variables and when conditioned on the other. However, it is evident that the probability of acceptance for $x_2$ increases more when $x_1$ is known than $x_1$ does when $x_2$ is known, suggesting the relative importance of $x_1$ compared to $x_2$.
...
Since the selection criteria are dependent on two different variables, it is possible that this regression and slope does not match the actual importance metrics.
A probability-based metric is used instead.
...
Compared to the other criteria, though, it shows that $x_1$ is even more important, as highlighted by the smaller variance. 
This is because $x_1$ has a narrower set of values that it can occupy and still have a probability of being accepted (roughly 0.8 to 2.6) compared to $x_2$ (roughly 0 to 3.2).
...
Consistent with the feature importance ranking, conditioning $x_1$ on $x_2$ has less improvement than conditioning $x_2$ on $x_1$.
...
The regressions and slopes are largely the same for all datasets due to the dominance of points from the original starting design and not the boundary refinement search, as well as being a reflection of the data and not the model.
...
ggplot() +
  ...
  geom_path(data = Infer.x2, mapping = aes(x = x, y = prob, color = 'x2')) +
  labs(x = expression('x'), y = 'Probability of Acceptance', 
       subtitle = expression('Accept if r < 1, f'[1]*' priority > 80%')) +
  scale_color_manual(name = 'Variable', values = c('x1' = 'red', 'x2' = 'blue'), 
                    labels = c('x1' = expression('x'[1]), 'x2' = expression('x'[2]))) +
  theme_bw()
...
This highlights the rationale for using percentiles instead of the variance, as the mean value sometimes does not fall between the 25th and 75th percentiles.
This indicates that the distribution is non-Gaussian, and therefore it is likely that the variance would reflect the skewness, potentially being higher than the mean.
Using percentiles is guaranteed to be a better reflection of the distribution, and therefore a better estimate for how much variability there is due to the random sampling in the Monte Carlo.
...
ggplot(rbind(Infer.x1, Infer.x2, Infer.x1.x2, Infer.x2.x1)) +
  ...
  labs(x = '', y = 'Probability of Acceptance', 
       subtitle = expression('Accept if r < 1, f'[1]*' priority < 80%'), color = '') +
  scale_y_continuous(expand = c(0, 0.05)) + scale_x_continuous(expand = c(0, 0)) +
  theme_bw()
...
##
# Grid of the relevant region to visualize
lower = c(0, 0); upper = c(3.5,3.5); grid.sz = 100 # Based on previous samples, no boundary is greater than 3.5
...
ggplot() +
  ...
  scale_x_continuous(expand = c(0, 0), limits = c(0, 5)) + scale_y_continuous(expand = c(0, 0), limits = c(0, 3.5)) +
