...
Normalization is defined such that (0,0) is the utopia point and (1,0) and (0,1) are the single-objective optimization. 
The prioritization is the angle $\theta$ made between the point in the normalized objective space and the x-axis, i.e. the axis of purely prioritizing objective 2. This angle is re-mapped to a percentage from 0 to 100% prioritization of objective 2. 
...
For this example, define $\delta$ as the distance between the point and a point on the Pareto frontier with the same value of theta. Accept only points with $\delta < 1$.
...
The total uncertainty function prioritizes points that are likely to describe the boundary (in this selection criterion: normalized distance equal to 1) with the greatest model uncertainty at the time. 
When a point is sampled, these uncertainties become zero because the result becomes known, indicating that the point that would lead to the most information gain would be the point where the total uncertainty is maximized.
...
The cutoff threshold is when the sample utility function's maximum falls below 5% of the sample utility of the first iteration, or when the budget has been expended.
For consistency with the Pareto frontier search, the budget is set to 20 (10 times the nubmer of dimensions).
...
The integral equates to a mean value, which can be estimated by Monte Carlo sampling. The variance in this estimate is the variance among the samples.
...
Due to the simplicity of the test system, the marginals all fall within the confidence interval of the expected probability from marginalization of the true function.

The importance ranking suggests that the first variable $x_1$ is more important than the second.
...
To account for possible uncertainty in the estimate of the optimal $x_1$, the conditional for $x_1$ will be within its optimal region, defined by the values of $x_1$ with at least half of the max probability.

For illustrative purposes, the other conditional will also be shown (accept | $x_1, x_2 = x_{2,opt}$). This may be useful if, for instance, $x_2$ is easier to measure.
...
As expected based on the importance ranking, conditioning $x_2$ on $x_1$ leads to a larger improvement in the probability of acceptance than the other way around.
...
The first criteria is analogous to a design problem with specific constraints that must be met, and the location of the Pareto frontier is irrelevant to these goals.
The second criteria defines a system with a predefined prioritization of objectives, and defines proximity to optimality not on the Pareto frontier, but on the utopia point.
...
For this acceptance criteria, the boundary of the classifier is well defined with just the Pareto frontier, so additional iterations are not as necessary. Sampling tended to prioritize exploration over exploitation for this reason.
Continuing with the procedure to show the feature importance and acceptance probability procedures.
...
Like the previous selection criteria, the marginals all generally fit within the confidence interval from direct evaluation of the function, so the marginals provide little insight into the benefits of adaptive sampling.

Evaluating the probabilities when conditioned on optimal values (suggested ranges), as above.
...
As with the previous selection criteria, conditioning $x_2$ on $x_1$ led to the greatest improvement in the probability, consistent with the importance ranking.

### Utopia Distance and Prioritization
Since the utopia point is set to (0,0), the distance to the utopia point is simple to calculate from the normalized coordinates. Acceptable points are those that prioritize the first objective by at least 20%, which can be determined by the angle theta.
...
Like the previous selection criteria, the improvement to the probabilities is related to the variable importance ranking.
Another note: the probabilities for $x_2$ show the fact that the probabilities should be multimodal, consistent with the description of the accepted region, which has a concavity in it associated with the prioritization of $f_1$ over $f_2$.
...
It appears that the absolute values are largely irrelevant, as there is no clear interpretation of what an importance metric of 2.5 means compared to a metric of 1.5.
The fact that these ratios are unbounded on the upper range indicates that comparison of relative importance for different systems should be normalized for comparison, and only the relative differences matter.
The difference among selection criteria indicates that the importance is not the importance a particular variable has on the outcome of the objectives (which is held constant), but rather the the outcome of the classifier.
...
* The variance is very large, despite using the same number of calculations as our method based on marginalization.
* The Shapley values show that the impact of all variables can be negative, indicating that on average, changing either x1 or x2 will lead to a decrease in the probability of being accepted.
* The relative magnitude of $x_1$ and $x_2$ are consistent with the previous importance rankings in that $x_1$ typically has a larger magnitude Shapley value.

Overall, the Shapley values are not particularly useful in determining which variable is most important for meeting the selection criterion because of the high variance.
