...
for(i in 1:resolution){
  ...
  Infer.x1 = rbind(Infer.x1, data.frame(x = x1.seq[i], prob = mean(fill.frame$p.del1), var = 'x1', cond = 0))
  ...
  res = predict(object = mod.dist, newdata = fill.frame, type = 'UK')
  fill.frame$p.del1 = pnorm(q = 0, mean = res$mean - 1, sd = res$sd)
  Infer.x2 = rbind(Infer.x2, data.frame(x = x2.seq[i], prob = mean(fill.frame$p.del1), var = 'x2', cond = 0))
  ...
  res = predict(object = mod.dist, newdata = fill.frame, type = 'UK')
  fill.frame$p.del1 = pnorm(q = 0, mean = res$mean - 1, sd = res$sd)
  Infer.x3 = rbind(Infer.x3, data.frame(x = x3.seq[i], prob = mean(fill.frame$p.del1), var = 'x3', cond = 0))
}
...
p.target3 = 0.5*max(Infer.x3$prob)
# Since the probability distribution for this function has a single peak, the easiest method is to find the cutoffs for this acceptance.
x3.lims = range(filter(Infer.x3, prob > p.target2)$x)
# Adjust the extremes if they are not already at the extreme
if(x3.lims[1] > min(Infer.x3$x)){
  pos = which.min(abs(Infer.x3$x - x3.lims[1]))
  sub = Infer.x3[c(pos-1, pos), ]
  mod = lm(x ~ prob, data = sub)
  res = predict(object = mod, newdata = data.frame(prob = p.target3))
  x3.lims[1] = res
  rm(mod)
}
if(x3.lims[2] < max(Infer.x3$x)){
  pos = which.min(abs(Infer.x3$x - x3.lims[2]))
  sub = Infer.x3[c(pos, pos+1), ]
  mod = lm(x ~ prob, data = sub)
  res = predict(object = mod, newdata = data.frame(prob = p.target3))
  x3.lims[2] = res
  rm(mod)
...
# Single conditional: x1|x2, x2|x1, x3 is unknown
...
for(i in 1:resolution){
  ...
  res = predict(object = mod.dist, newdata = fill.frame, type = 'UK')
  fill.frame$p.del1 = pnorm(q = 0, mean = res$mean - 1, sd = res$sd)
  Infer.x2.x1 = rbind(Infer.x2.x1, data.frame(x = x2.seq[i], prob = mean(fill.frame$p.del1), var = 'x2', cond = 1))
  # x2 | x1, x3
  fill.frame = data.frame(x2 = x2.seq[i], x1 = runif(n = MCsamp, min = min(x1.lims), max = max(x1.lims)),
  ...
  Infer.x2.x1.x3 = rbind(Infer.x2.x1.x3, data.frame(x = x2.seq[i], 
                                                    prob = mean(fill.frame$p.del1), var = 'x2', cond = 2))
  # x1 | x2
  fill.frame = data.frame(x1 = x1.seq[i], x2 = runif(n = MCsamp, min = min(x2.lims), max = max(x2.lims)),
                          x3 = runif(n = MCsamp, min = min(x3.rng), max = max(x3.rng)))
  res = predict(object = mod.dist, newdata = fill.frame, type = 'UK')
  fill.frame$p.del1 = pnorm(q = 0, mean = res$mean - 1, sd = res$sd)
  Infer.x1.x2 = rbind(Infer.x1.x2, data.frame(x = x1.seq[i], prob = mean(fill.frame$p.del1), var = 'x1', cond = 1))
  # x1 | x2, x3
  fill.frame = data.frame(x1 = x1.seq[i], x2 = runif(n = MCsamp, min = min(x2.lims), max = max(x2.lims)),
                          x3 = runif(n = MCsamp, min = min(x3.lims), max = max(x3.lims)))
  res = predict(object = mod.dist, newdata = fill.frame, type = 'UK')
  ...
  Infer.x1.x2.x3 = rbind(Infer.x1.x2.x3, data.frame(x = x1.seq[i], 
                                                    prob = mean(fill.frame$p.del1), var = 'x1', cond = 2))

}

# Combine results for visualization
Infer.plt = rbind(Infer.x1, Infer.x2, Infer.x1.x2, Infer.x1.x2.x3, Infer.x2.x1, Infer.x2.x1.x3);
