...
# Description
This notebook showcases the method below for finding acceptable suboptimal performance in the first rest function form Zitzler, Deb, and Thiele, 2000.
This first function produces a concave Pareto frontier and takes up to 6 inputs.
...
7. Calculate the conditional probability that a point will fall within the near-Pareto set given only information about the most important input variable.
8. Calculate the 25th and 75th quantile of the 1-variable conditional probability estimate. The area between these two curves represents the variance due to all of the other variables in question.
9. Determine the normalized areas between these quantiles for all 1-variable conditionals. The most important variable is the one that has the smallest area, i.e. the other variables have small impact on whether or not the result meets the selection criteria.
...
The function ZDT1 takes N inputs, each on the domain of (0,1). The function is built into the GPareto package and does not need to be constructed explicitly.
...
res = easyGParetoptim(fn = ZDT1, budget = Pareto.budget, lower = c(0, 0, 0), upper = c(1, 1, 1), 
...
n.obj = function(GPar.data, GPar.front){
  ...
  GPar.data$f1.norm = (GPar.data$f1 - min(GPar.front$f1))/f1.up
  GPar.data$f2.norm = (GPar.data$f2 - min(GPar.front$f2))/f2.up
...
gplt.start = ggplot() +
  ...
  geom_point(data = filter(GPar.all, order == 0), mapping = aes(x = f1, y = f2, color = (order > 0))) +
...
gplt.Norm = ggplot() +
  ...
  scale_color_discrete(labels = c('Starting Data', 'Pareto Search'), breaks = c(FALSE, TRUE)) +
  scale_y_continuous(breaks = c(seq(from = 0, to = 10, by = 2), 1)) +
  theme_classic() + theme(legend.position = c(0.85, 0.85), legend.background = element_rect(fill = 'white'), legend.title = element_blank())
...
g1 = ggplot() +
  geom_contour_filled(data = filter(fine.grid, x3 == GPar.new$x3, x2 != GPar.new$x2), 
                      mapping = aes(x = x1, y = x2, z = (dist.p1)*(1-dist.p1)-1e-10,
  ...
  geom_contour(data = filter(fine.grid, x3 == GPar.new$x3, x2 != GPar.new$x2), 
               mapping = aes(x = x1, y = x2, z = dist.p1), breaks = c(0.25, 0.75), color = 'blue') +
...
g2 = ggplot() +
  geom_contour_filled(data = filter(fine.grid, x3 == GPar.new$x3, x2 != GPar.new$x2), 
                      mapping = aes(x = x1, y = x2, z = dist.sd, color = dist.sd), bins = ncolor, alpha = 0.5)+
  geom_point(data = filter(GPar.all, x1 <= upper[1], x1 >= lower[1], x2 <= upper[2], x2 >= lower[2], order == 0),
             mapping = aes(x = x1, y = x2, color = 'initial', shape = 'initial'), size = pt.size) +
  geom_point(data = filter(GPar.all, x1 <= upper[1], x1 >= lower[1], x2 <= upper[2], x2 >= lower[2], order > 0),
             mapping = aes(x = x1, y = x2, color = 'Pareto', shape = 'Pareto'), size = pt.size) +
...
g3 = ggplot() +
  geom_contour_filled(data = filter(fine.grid, x3 == GPar.new$x3, x2 != GPar.new$x2), 
                      mapping = aes(x = x1, y = x2, z = dist.sd*(dist.p1)*(1-dist.p1), color = dist.sd*(dist.p1)*(1-dist.p1)),
...
g1 = ggplot() +
  geom_contour_filled(data = filter(fine.grid, x2 == GPar.new$x2, x3 != GPar.new$x3), 
                      mapping = aes(x = x1, y = x3, z = (dist.p1)*(1-dist.p1)-1e-10,
                                    color = (dist.p1)*(1-dist.p1)-1e-10), alpha = 0.5,
                      breaks = seq(from = -0.01, to = 0.25, length.out = ncolor)
                      )+
  geom_contour(data = filter(fine.grid, x2 == GPar.new$x2, x3 != GPar.new$x3),
               mapping = aes(x = x1, y = x3, z = dist.p1), breaks = c(0.25, 0.75), color = 'blue') +
  geom_point(data = filter(GPar.all, x1 <= upper[1], x1 >= lower[1], x2 <= upper[2], x2 >= lower[2], order == 0),
...
g2 = ggplot() +
  geom_contour_filled(data = filter(fine.grid, x2 == GPar.new$x2, x3 != GPar.new$x3), 
                      mapping = aes(x = x1, y = x3, z = dist.sd, color = dist.sd), bins = ncolor, alpha = 0.5)+
  geom_point(data = filter(GPar.all, x1 <= upper[1], x1 >= lower[1], x2 <= upper[2], x2 >= lower[2], order == 0),
             mapping = aes(x = x1, y = x3, color = 'initial', shape = 'initial'), size = pt.size) +
  geom_point(data = filter(GPar.all, x1 <= upper[1], x1 >= lower[1], x2 <= upper[2], x2 >= lower[2], order > 0),
             mapping = aes(x = x1, y = x3, color = 'Pareto', shape = 'Pareto'), size = pt.size) +
...
g3 = ggplot() +
  geom_contour_filled(data = filter(fine.grid, x2 == GPar.new$x2, x3 != GPar.new$x3), 
                      mapping = aes(x = x1, y = x3, z = dist.sd*(dist.p1)*(1-dist.p1), color = dist.sd*(dist.p1)*(1-dist.p1)),
                      bins = ncolor, alpha = 0.5)+
  geom_point(data = filter(GPar.all, x1 <= upper[1], x1 >= lower[1], x2 <= upper[2], x2 >= lower[2], order == 0),
             mapping = aes(x = x1, y = x3, color = 'initial', shape = 'initial'), size = pt.size) +
  geom_point(data = filter(GPar.all, x1 <= upper[1], x1 >= lower[1], x2 <= upper[2], x2 >= lower[2], order > 0),
...
## III. Feature importance of each input variable
The easiest and typical method of feature importance is to take the correlation coefficient between the input and the output.
For a bi-objective problem, dimension reduction to a single output for the correlation coefficient is not straightforward.
In this case, the approximation is to use to the relative distance from the Pareto front and compare it to the relative distance in the input space.
...
The regressions highlight the symmetry of $x_2$ and $x_3$ in their nearly identical slopes and correlation coefficients.
It also shows that $x_1$ has the weakest relationship, indicated by its lower correlation and slope.
However, using these normalized distances is only useful when that is the goal; in this case, the goal is to determine whether or not a point meets the acceptance criterion, which is obscured in the slope and correlation metrics.

7. Calculate the conditional probability that a point will fall within the near-Pareto set given only information about the most important input variable.

8. Calculate the standard deviation of the 1-variable conditional probability estimate. The area between these two curves represents the variance due to all of the other variables in question.

A substantial fraction of the randomly sampled probabilities are 0 or 1 because the optimal region is so small compared to the total search space. 
As a result, a percentile-based estimate of variance as in the 2-variable polynomial cases would give an area of 0, or very close to 0, unless more samples were collected.
While the variance has little physical relevance (because the distribution is non-normal), it still gives a good indication of the variance in probability caused by the other input variables, and therefore works as a sufficient metric for feature importance ranking.

9. Determine the normalized areas between these percentiles for all 1-variable conditionals. The most important variable is the one that has the smallest area, i.e. the other variables have small impact on whether or not the result meets the selection criteria.
...
ggplot() +
  ...
  labs(x = expression('x'), y = 'Probability of Acceptance', 
       subtitle = expression('Accept if '*delta*' < 1')) +
  scale_color_manual(name = 'Variable', values = c('x1' = 'red', 'x2' = 'blue', 'x3' = 'green'), 
                    labels = c('x1' = expression('x'[1]), 'x2' = expression('x'[2]),'x3' = expression('x'[3]))) +
  theme_bw()

# Plot with percentiles
ggplot() +
  # Percentiles as an area
  geom_polygon(data = data.frame(x = c(Infer.x1$x, rev(Infer.x1$x)),
                              y = c(Infer.x1$prob, rev(Infer.x1$prob)),
                              sd = c(Infer.x1$psd, -rev(Infer.x1$psd))),
            mapping = aes(x = x, y = y+sd, fill = 'x1'), alpha = 0.25) +
  geom_polygon(data = data.frame(x = c(Infer.x2$x, rev(Infer.x2$x)),
                              y = c(Infer.x2$prob, rev(Infer.x2$prob)),
                              sd = c(Infer.x2$psd, -rev(Infer.x2$psd))),
            mapping = aes(x = x, y = y+sd, fill = 'x2'), alpha = 0.25) +
  geom_polygon(data = data.frame(x = c(Infer.x3$x, rev(Infer.x3$x)),
                              y = c(Infer.x3$prob, rev(Infer.x3$prob)),
                              sd = c(Infer.x3$psd, -rev(Infer.x3$psd))),
            mapping = aes(x = x, y = y+sd, fill = 'x3'), alpha = 0.25) +

  # Mean values as a line
  geom_path(data = Infer.x1, mapping = aes(x = x, y = prob, color = 'x1')) +
  geom_path(data = Infer.x2, mapping = aes(x = x, y = prob, color = 'x2')) +
  geom_path(data = Infer.x3, mapping = aes(x = x, y = prob, color = 'x3')) +
  labs(x = expression('x'), y = 'Probability of Acceptance', subtitle = expression('Accept if '*delta*' < 1')) +
  scale_color_manual(name = 'Variable', values = c('x1' = 'red', 'x2' = 'blue', 'x3' = 'green'), 
                    labels = c('x1' = expression('x'[1]), 'x2' = expression('x'[2]),'x3' = expression('x'[3]))) +
  scale_fill_manual(name = 'Variable', values = c('x1' = 'red', 'x2' = 'blue', 'x3' = 'green'), 
                    labels = c('x1' = expression('x'[1]), 'x2' = expression('x'[2]),'x3' = expression('x'[3]))) +
  theme_bw() + guides(fill = FALSE)

# Relative importance: inverse of the area under the curve, normalized units
ggplot(data.frame(import = c(resolution/sum(Infer.x1$p75 - Infer.x1$p25), 
                             resolution/sum(Infer.x2$p75 - Infer.x2$p25),
                             resolution/sum(Infer.x3$p75 - Infer.x3$p25)),
           var = c('x1', 'x2', 'x3'))) +
  geom_col(mapping = aes(x = var, y = import, fill = var)) +
  labs(x = '', y = 'Relative Importance', fill = 'Variable', 
       subtitle = expression('Accept if '*delta*' < 1; Percentiles')) +
  scale_y_continuous(breaks = 0) + scale_x_discrete(labels = c('', '')) + 
  scale_fill_manual(values = c('red', 'blue', 'green'))

ggplot(data.frame(import = c(resolution/sum(Infer.x1$psd*2), 
                             resolution/sum(Infer.x2$psd*2),
                             resolution/sum(Infer.x3$psd*2)),
           var = c('x1', 'x2', 'x3'))) +
  geom_col(mapping = aes(x = var, y = import, fill = var)) +
  labs(x = '', y = 'Relative Importance', fill = 'Variable', 
       subtitle = expression('Accept if '*delta*' < 1; Variance')) +
  scale_y_continuous(breaks = 0) + scale_x_discrete(labels = c('', '', '')) + 
  scale_fill_manual(values = c('red', 'blue', 'green'))
...
Infer.x3 = Infer.x3[,!names(Infer.x3) %in% c('p75', 'p25', 'psd')]
```

As shown above: basing the results on the percentiles over-inflates the importance of $x_1$ because the range of its 25th to 75th percentile only captures probabilties of 0, giving a normalized area of 0.
...
# Ranges of 'good' results. All results only have a single peak, so it is a simple constraint.
x1.lims = p.lims(data.prob = Infer.x1, p.target = 0.5*max(Infer.x1$prob))
x2.lims = p.lims(data.prob = Infer.x2, p.target = 0.5*max(Infer.x2$prob))
x3.lims = p.lims(data.prob = Infer.x3, p.target = 0.5*max(Infer.x3$prob))

# Marginalize over the specific region of x1 and x2. Due to similarities in x2 and x3, only testing x2.
resolution = 50; MCsamp = 1500
x1.seq = seq(from = min(x1.rng), to = max(x1.rng), length.out = resolution)
x2.seq = seq(from = min(x2.rng), to = max(x2.rng), length.out = resolution)
# x3.seq = seq(from = min(x3.rng), to = max(x3.rng), length.out = resolution)

# Single conditional: x1|x2, x2|x1, x1|x3, x2|x3
Infer.x2.x1 = data.frame(); Infer.x1.x2 = data.frame(); 
Infer.x2.x3 = data.frame(); Infer.x1.x3 = data.frame(); 
# Full conditional: x1|x2,x3 and x2|x1,x3
Infer.x1.x2.x3 = data.frame(); Infer.x2.x1.x3 = data.frame()
...
for(i in 1:resolution){
  ...
  # x2 | x3
  fill.frame = data.frame(x2 = x2.seq[i], x1 = runif(n = MCsamp, min = min(x1.rng), max = max(x1.rng)),
                          x3 = runif(n = MCsamp, min = min(x3.lims), max = max(x3.lims)))
  res = predict(object = mod.dist, newdata = fill.frame, type = 'UK')
  fill.frame$p.del1 = pnorm(q = 0, mean = res$mean - 1, sd = res$sd)
  Infer.x2.x3 = rbind(Infer.x2.x1, data.frame(x = x2.seq[i], prob = mean(fill.frame$p.del1), var = 'x2', cond = '|x3'))
  # x2 | x1, x3
  fill.frame = data.frame(x2 = x2.seq[i], x1 = runif(n = MCsamp, min = min(x1.lims), max = max(x1.lims)),
                          x3 = runif(n = MCsamp, min = min(x3.lims), max = max(x3.lims)))
  res = predict(object = mod.dist, newdata = fill.frame, type = 'UK')
  fill.frame$p.del1 = pnorm(q = 0, mean = res$mean - 1, sd = res$sd)
  Infer.x2.x1.x3 = rbind(Infer.x2.x1.x3, data.frame(x = x2.seq[i], 
                                                    prob = mean(fill.frame$p.del1), var = 'x2', cond = '|x1, x3'))
  # x1 | x2
  fill.frame = data.frame(x1 = x1.seq[i], x2 = runif(n = MCsamp, min = min(x2.lims), max = max(x2.lims)),
                          x3 = runif(n = MCsamp, min = min(x3.rng), max = max(x3.rng)))
  res = predict(object = mod.dist, newdata = fill.frame, type = 'UK')
  fill.frame$p.del1 = pnorm(q = 0, mean = res$mean - 1, sd = res$sd)
  Infer.x1.x2 = rbind(Infer.x1.x2, data.frame(x = x1.seq[i], prob = mean(fill.frame$p.del1), var = 'x1', cond = '|x2'))
  # x1 | x3
  fill.frame = data.frame(x1 = x1.seq[i], x2 = runif(n = MCsamp, min = min(x2.rng), max = max(x2.rng)),
                          x3 = runif(n = MCsamp, min = min(x3.lims), max = max(x3.lims)))
  res = predict(object = mod.dist, newdata = fill.frame, type = 'UK')
  fill.frame$p.del1 = pnorm(q = 0, mean = res$mean - 1, sd = res$sd)
  Infer.x1.x3 = rbind(Infer.x1.x2, data.frame(x = x1.seq[i], prob = mean(fill.frame$p.del1), var = 'x1', cond = '|x3'))
  # x1 | x2, x3
  fill.frame = data.frame(x1 = x1.seq[i], x2 = runif(n = MCsamp, min = min(x2.lims), max = max(x2.lims)),
                          x3 = runif(n = MCsamp, min = min(x3.lims), max = max(x3.lims)))
  res = predict(object = mod.dist, newdata = fill.frame, type = 'UK')
  fill.frame$p.del1 = pnorm(q = 0, mean = res$mean - 1, sd = res$sd)
  Infer.x1.x2.x3 = rbind(Infer.x1.x2.x3, data.frame(x = x1.seq[i], 
                                                    prob = mean(fill.frame$p.del1), var = 'x1', cond = '|x2, x3'))

}

# Combine results for visualization
Infer.plt = rbind(Infer.x1, Infer.x2, Infer.x1.x2, Infer.x1.x2.x3, Infer.x2.x1, Infer.x2.x1.x3, Infer.x2.x3, Infer.x2.x3);
...
ggplot(Infer.plt) +
  geom_line(mapping = aes(x = x, y = prob, color = as.factor(cond))) +
...
Due to symmetry, conditioning $x_1$ on either $x_2$ or $x_3$ gives no difference.
Additionally, conditioning $x_2$ on $x_1$ gives little change, but conditioning it on $x_3$ substantially improves the probabilities.
The change upon conditioning $x_2$ on $x_1$ is so small that those it effectively does not change, reflecting the low importance of $x_1$ and supporting the use of the variance over the percentiles in the importance ranking.
...
The importance of individual variables is largely the same, which makes sense given that most of the data used in this analysis is from the initial design and Pareto search, not the post-processing.
...
ggplot() +
  ...
  scale_color_manual(name = 'Variable', values = c('x1' = 'red', 'x2' = 'blue', 'x3' = 'black'), 
                    labels = c('x1' = expression('x'[1]), 'x2' = expression('x'[2]),'x3' = expression('x'[3]))) +
  theme_bw()
...
ggplot(data.frame(import = c(resolution/sum(Infer.x1$p75 - Infer.x1$p25), 
  ...
  geom_col(mapping = aes(x = var, y = import, fill = var)) +
  labs(x = '', y = 'Relative Importance', fill = 'Variable', 
       subtitle = expression('Accept if f'[1]*' < 1 & f'[2]*' < 1; Percentiles')) +
  scale_y_continuous(breaks = 0) + scale_x_discrete(labels = c('', '', '')) + 
  scale_fill_manual(values = c('red', 'blue', 'green'))
...
Like with the previous selection criterion, using the percentiles over-inflated the importance of $x_1$ due to undersampling from its distribution only capturing the points at or near 0.
...
# Marginalize over the specific region of x1 and x2. Due to similarities in x2 and x3, leaving that alone.
resolution = 50; MCsamp = 1500
x1.seq = seq(from = min(x1.rng), to = max(x1.rng), length.out = resolution)
x2.seq = seq(from = min(x2.rng), to = max(x2.rng), length.out = resolution)
# x3.seq = seq(from = min(x3.rng), to = max(x3.rng), length.out = resolution)

# Single conditional: x1|x2, x2|x1, etc.
Infer.x2.x1 = data.frame(); Infer.x1.x2 = data.frame(); 
Infer.x2.x3 = data.frame(); Infer.x1.x3 = data.frame(); 
# Full conditional: x1|x2,x3 and x2|x1,x3
Infer.x1.x2.x3 = data.frame(); Infer.x2.x1.x3 = data.frame()
...
for(i in 1:resolution){
  ...
  fill.frame$p.del1 = pnorm(q = 0, mean = res1$mean - 1, sd = res1$sd)*pnorm(q = 0, mean = res2$mean - 1, sd = res2$sd)
  Infer.x1.x2.x3 = rbind(Infer.x1.x2.x3, data.frame(x = x1.seq[i], 
                                                    prob = mean(fill.frame$p.del1), var = 'x1', cond = '|x1, x2'))

}
rm(res1, res2, fill.frame)
# Combine results for visualization
Infer.plt = rbind(Infer.x1, Infer.x2, Infer.x1.x2, Infer.x1.x2.x3, Infer.x2.x1, Infer.x2.x1.x3, Infer.x1.x3, Infer.x2.x3);
...
ggplot(Infer.plt) +
  geom_line(mapping = aes(x = x, y = prob, color = as.factor(cond))) +
  facet_wrap(var~., nrow = 2) + 
  labs(x = '', y = 'Probability of Acceptance', expression('Accept if f'[1]*'* < 1, f'[2]*'* < 1'), 
       color = 'Partial Conditions') +
  scale_y_continuous(expand = c(0, 0.05)) + scale_x_continuous(expand = c(0, 0)) +
  theme_bw()
...
ggplot(data.frame(import = c(resolution/sum(Infer.x1$p75 - Infer.x1$p25), 
  ...
  geom_col(mapping = aes(x = var, y = import, fill = var)) +
  labs(x = '', y = 'Relative Importance', fill = 'Variable', 
       subtitle = expression('Accept if r < 1 & > 80% f'[1]*' priority; Percentiles')) +
  scale_y_continuous(breaks = 0) + scale_x_discrete(labels = c('', '')) + 
  scale_fill_manual(values = c('red', 'blue', 'green'))
...
Unlike the other selection criteria, this criteria obtained the correct order with the percentiles, although the relative magnitudes were incorrect.
However, it is unclear at the moment if the relative magnitudes matter outside of ordering.
Given the consistency of using the variance, for the metric (as well as the ease of its calculation), the variance will be used in place of percentiles for problems with at least 3 input variables.
